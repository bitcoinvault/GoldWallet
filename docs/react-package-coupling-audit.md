...
- Latest React patch checked on 2026-05-31: `19.2.6`
...
- React `19.2.6` remains blocked on this RN `0.85.3` baseline because `react-native-renderer` is exact-version sensitive at `19.2.3`.

## Coupling Rules

- Do not update React without updating `react-test-renderer` and `@types/react` in the same React/RN baseline branch.
- Do not take the React `19.2.6` patch on this RN `0.85.3` line unless a dedicated runtime proof also moves or validates the matching React Native renderer.
