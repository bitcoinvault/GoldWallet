...
After `BEM-37.284`, the same audit also reports `ios/Podfile.lock` drift against the current native package baseline. On this Windows machine the lockfile still references stale pods such as React Native `0.65.3`, removed `react-native-camera`, removed `react-native-qrcode-local-image`, older Firebase, Sentry, BootSplash, Config, AsyncStorage, DeviceInfo, GestureHandler, Localize, Screens, and VectorIcons pods. The static iOS project files are valid, but iOS archive readiness is not claimable until `pod install` refreshes `ios/Podfile.lock` on macOS and an affected scheme builds.
