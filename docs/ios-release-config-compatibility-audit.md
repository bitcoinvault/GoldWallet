...
After `BEM-37.794`, `corepack yarn check:ios-release-config-doc-guard` keeps this audit aligned with the current iOS release-config posture. Current CodePush posture is removed: iOS Info.plist files no longer contain native `CodePushDeploymentKey` placeholders, CodePush native/runtime integration is removed, and release-config follow-up must not reintroduce CodePush bundle/deployment-key work as a default iOS validation path. `ios/Podfile.lock` still has `12` active drift issues, so iOS runtime delivery validation remains not claimed until macOS with Xcode `26.2+`, CocoaPods, and a refreshed `ios/Podfile.lock` are available. Use `ios:mac-validation:handoff:preflight --all-schemes` on Windows to refresh static evidence, then run `ios:mac-validation:handoff --all-schemes` on macOS for complete shared-scheme simulator/archive validation.

After `BEM-37.961`, the selected Firebase Apple SDK `12.17.0` requires Xcode `26.2+`, which is stricter than React Native `0.86.2`'s own `16.1` minimum. The standalone release-readiness audit, macOS prerequisite audit, and Podfile refresh plan record both dependency floors and compute `26.2` as the effective minimum. Both the executable handoff and generated refresh plan rerun the prerequisite audit, validate its fresh summary, and then run a toolchain-only fail-fast gate before `pod install`; this gate permits the expected lockfile drift but rejects non-macOS hosts, Xcode below `26.2`, and unavailable CocoaPods before they can mutate `ios/Podfile.lock`. The guarded `macos-15-intel` workflow selects `/Applications/Xcode_26.2.app` explicitly.
...
After `BEM-37.958.1`, root `Gemfile.lock` freezes the Ruby 3.3.12/Bundler 4.0.16 CocoaPods graph for the Intel Darwin runner and CI enables Bundler frozen mode. The workflow writes `local-docs/ios-macos-ci-execution-summary.txt`, while the project handoff writes per-step results to `local-docs/ios-mac-validation-execution-summary.txt` and retains its complete log before propagating failure. An `xcodebuild` failure therefore identifies the failed scheme and exit code even though pre-build readiness summaries are retained for diagnosis. The workflow guard accepts both LF and CRLF checkouts and verifies the lockfile, frozen Bundler mode, execution outcome evidence, and existing fail-closed controls.
