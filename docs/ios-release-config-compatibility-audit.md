...
After `BEM-37.333`, the same audit reports `ios/Podfile.lock` drift against the current native package baseline on 2026-06-03. On this Windows machine the lockfile still references React Native `0.65.3`, removed `RNCMaskedView`, removed Flipper pods, and older BootSplash, Config, AsyncStorage, DeviceInfo, FastImage, Firebase, Gesture Handler, Localize, Screens, Sentry, and VectorIcons pods. The static iOS project files are valid, but iOS archive readiness is not claimable until `pod install` refreshes `ios/Podfile.lock` on macOS and an affected scheme builds.
