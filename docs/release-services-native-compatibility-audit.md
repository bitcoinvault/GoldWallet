...
| `@sentry/react-native` | `8.19.0` | `8.19.0` on 2026-07-21 | Latest checked SDK line; source-map and dSYM upload must still be proven with local credentials. |
...
- Sentry `8.19.0` keeps the Android Gradle/source-map wiring visible and no active Sentry `execResult` warning is reported on the RN `0.86.0` baseline. The repo-owned legacy args shim lets Sentry read release bundle output and source-map output from RN `0.86.0` bundle tasks; Android release validation proves source-map generation for `dev`, `stage`, `prod`, and `beta`. Release artifact upload still needs credentials before it can be claimed as fully validated.
