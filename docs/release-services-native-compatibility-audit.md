...
| `@sentry/react-native` | `8.18.0` | `8.18.0` on 2026-07-10 | Latest checked SDK line; source-map and dSYM behavior must still be proven with local credentials. |
| `@sentry/cli` | `3.6.0` | `3.6.0` on 2026-07-05 | Explicit release-tooling dependency; prerequisite audit checks binary availability, live latest metadata, installed direct/nested CLI versions, and whether release build phases use the direct root CLI package. |
...
- A focused 2026-07-10 Sentry package/prerequisite refresh reran live npm checks, Android warning/source-map wiring audits, RN bundle-task compatibility, release prerequisite, credential-plan, Android release build validation, release no-network smoke, iOS static handoff checks, and aggregate release-services summary validation against refreshed Android release evidence. It does not claim credentialed upload; it confirms `@sentry/react-native@8.18.0` and direct `@sentry/cli@3.6.0` are current, Android warning wiring is valid, RN bundle-task compatibility remains ready through the repo-owned shim, release-services remains valid only under the controlled Electrum certificate blocker, `Sentry release upload validation: not claimed`, missing properties files `3`, `SENTRY_AUTH_TOKEN available in current shell: no`, iOS macOS archive validation not ready, active `ios/Podfile.lock` drift issues `12` including `RNScreens 3.6.0` versus package `4.26.0`, and `Secret values printed: no`.
...
- `@sentry/react-native` is on latest checked `8.18.0` after the Sentry SDK upgrade; Android build and smoke validation are required for any future runtime/package Sentry branch, while source-map/dSYM upload remains blocked locally until Sentry credentials/properties are available and the credentialed release runner proves upload.
