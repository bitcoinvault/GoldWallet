...
| `@sentry/react-native` | `8.16.0` | `8.16.0` on 2026-06-29 | Latest checked SDK line; source-map and dSYM behavior must still be proven with local credentials. |
| `@sentry/cli` | `3.6.0` | `3.6.0` on 2026-06-29 | Explicit release-tooling dependency; prerequisite audit checks binary availability, live latest metadata, installed direct/nested CLI versions, and whether release build phases use the direct root CLI package. |
...
- Sentry SDK and CLI package targets remain current on 2026-06-29: `@sentry/react-native@8.16.0` and direct `@sentry/cli@3.6.0`.
...
- `@sentry/react-native` is on latest checked `8.16.0` after the Sentry SDK upgrade; Android build and smoke validation are required for any future runtime/package Sentry branch, while source-map/dSYM upload remains blocked locally until Sentry credentials/properties are available and the credentialed release runner proves upload.
