...
- Sentry production release/source-map upload validation remains unclaimed. The authenticated managed CLI can prepare all three ignored properties files, and the no-upload production preflight proves current Android runtime evidence, but iOS source-map/dSYM delivery still requires the guarded macOS/Xcode/CocoaPods archive path.
- Sentry SDK and CLI package targets are current on 2026-08-10: `@sentry/react-native@8.22.0` and direct `@sentry/cli@3.6.2`.
- The Sentry prerequisite audit records per-file readiness for the root, Android, and iOS Sentry properties files, validates that `create-sentry-properties.sh` writes all three expected paths for an explicit `SENTRY_RELEASE_PROFILE=nonprod|prod`, supports `SENTRY_ORG` plus platform-specific project overrides, verifies that the local direct `@sentry/cli` package binary is present and executable, requires the aggregate and nested version lists to exactly match discovered installations, and verifies that the Android resolver plus iOS build phases use the direct root CLI package.
- Sentry `8.22.0` keeps the Android Gradle/source-map wiring visible and no active Sentry `execResult` warning is reported on the RN `0.87.0` baseline. The repo-owned legacy args shim lets Sentry read release bundle output and source-map output from RN `0.87.0` bundle tasks; Android release validation proves source-map generation for `dev`, `stage`, `prod`, and `beta`. Release artifact upload remains unclaimed until the cross-platform production delivery gate passes.
...
- iOS static release files remain valid for RN `0.87.0`, minimum iOS `15.1`, and 8 guarded schemes, but macOS archive readiness remains blocked because this Windows machine has no `xcodebuild`, no CocoaPods, and `ios/Podfile.lock` currently has 13 active drift entries after the RN/package upgrades.
...
- Sentry changes can affect release bundling, source-map upload, dSYM upload, DSN handling, and Android Gradle integration even though the active RN `0.87.0` warning audit no longer reports Sentry `execResult`.
- `@sentry/react-native` is on latest checked `8.22.0`, with direct `@sentry/cli@3.6.2`; Android build and smoke validation are required for any future runtime/package Sentry branch, while production source-map/dSYM upload remains blocked until the credentialed Android and macOS/iOS release paths prove delivery.
- `corepack yarn sentry:release:prereq-audit` reports whether local `sentry.properties`, `android/sentry.properties`, `ios/sentry.properties`, and `SENTRY_AUTH_TOKEN` are available before Sentry release/source-map validation, rejects properties outside the guarded DecentraPlanet Android/iOS project sets, verifies explicit release-profile generation and platform-specific overrides, verifies the local direct `@sentry/cli` package/bin, records direct and nested Sentry CLI package versions, checks that Android/iOS release build paths use the direct root CLI package, checks latest local Android `dev`/`stage`/`prod`/`beta` release summary evidence plus current release-input coverage, embeds iOS static/macOS prerequisite evidence including active `ios/Podfile.lock` drift, and writes `local-docs/sentry-release-prereq-summary.txt`. The prerequisite status can only be `ready` when the Android release summary is present, covers dev/stage/prod/beta, matches current release inputs, has valid APK manifest proof, the Sentry CLI release path points at the direct root package, iOS macOS archive validation is ready, and `ios/Podfile.lock` drift is zero.
...
- Secrets: do not guess `SENTRY_AUTH_TOKEN`, project overrides, CodePush deployment keys, or Firebase config values. If missing locally, record the exact missing variable/file.
