...
- Sentry `8.22.0` keeps the Android Gradle/source-map wiring visible and no active Sentry `execResult` warning is reported on the RN `0.86.2` baseline. The repo-owned legacy args shim lets Sentry read release bundle output and source-map output from RN `0.86.2` bundle tasks; Android release validation proves source-map generation for `dev`, `stage`, `prod`, and `beta`. Release artifact upload remains unclaimed until the cross-platform production delivery gate passes.
