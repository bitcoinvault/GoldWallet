...
| --- | --- | --- | --- |
| `@react-native-firebase/app` | `25.0.1` | `25.0.1` on 2026-06-24 | Current package pulls `firebase@12.15.0`. |
| `@react-native-firebase/analytics` | `25.0.1` | `25.0.1` on 2026-06-24 | Peer requires matching `@react-native-firebase/app@25.0.1`. |
| `@react-native-firebase/crashlytics` | `25.0.1` | `25.0.1` on 2026-06-24 | Peer requires matching `@react-native-firebase/app@25.0.1`. |
| `@react-native-firebase/messaging` | `25.0.1` | `25.0.1` on 2026-06-24 | Peer requires matching `@react-native-firebase/app@25.0.1`. |
...
| `@sentry/react-native` | `8.15.1` | `8.15.1` on 2026-06-24 | Latest checked SDK line; source-map and dSYM behavior must still be proven with local credentials. |
| `@sentry/cli` | `3.5.1` | `3.5.1` on 2026-06-24 | Explicit release-tooling dependency; prerequisite audit checks binary availability, live latest metadata, installed direct/nested CLI versions, and whether release build phases use the direct root CLI package. |
...
- A focused 2026-06-24 Sentry preflight refresh reran live npm checks, Android warning/source-map wiring audits, RN bundle-task compatibility, release prerequisite, credential-plan, non-secret preflight, and aggregate release-services summary validation against the existing Android release evidence. It did not rebuild APKs or claim credentialed upload; it confirms `@sentry/react-native@8.15.1` and `@sentry/cli@3.5.1` remain current, Android warning wiring is valid, RN bundle-task compatibility is ready through the repo-owned shim, Android release/smoke/create-wallet evidence is ready, `Sentry release upload validation: not claimed`, missing properties files `3`, `SENTRY_AUTH_TOKEN available in current shell: no`, iOS macOS archive validation not ready, active `ios/Podfile.lock` drift issues `12`, and `Secret values printed: no`.
...
- Firebase release-services wiring is valid for the current `25.0.1` package family, Android config, iOS plist files, and Messaging runtime paths.
...
- Sentry SDK and CLI package targets remain current on 2026-06-24: `@sentry/react-native@8.15.1` and direct `@sentry/cli@3.5.1`.
...
- Sentry `8.15.1` keeps the Android Gradle/source-map wiring visible and no active Sentry `execResult` warning is reported on the RN `0.86.0` baseline. The repo-owned legacy args shim lets Sentry read release bundle output and source-map output from RN `0.86.0` bundle tasks; Android release validation must still prove source-map generation for `dev`, `stage`, `prod`, and `beta` after each SDK bump. Release artifact upload still needs credentials before it can be claimed as fully validated.
...
- Android Firebase package versions are now supplied by React Native Firebase `25.0.1`; the old manual `firebase-core:16.0.3` and app-level Firebase BoM `28.2.0` entries were removed to avoid duplicate measurement classes.
...
- Firebase RN `25.0.1` is the current package family and must stay aligned across app, analytics, Crashlytics, and messaging.
...
- `@sentry/react-native` is on latest checked `8.15.1` after the Sentry SDK upgrade; Android build and smoke validation are required for any future runtime/package Sentry branch, while source-map/dSYM upload remains blocked locally until Sentry credentials/properties are available and the credentialed release runner proves upload.
