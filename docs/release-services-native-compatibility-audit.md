...
The 2026-06-17 Firebase runtime-delivery readiness refresh confirms `@react-native-firebase/app`, `analytics`, `crashlytics`, and `messaging` are still aligned and current at `24.1.1`, and `@react-native-community/push-notification-ios` is still current at `1.12.0`. `firebase:runtime:delivery:handoff --skip-android-release` passes against current Android release build, manifest, release-smoke, and release create-wallet evidence. Real FCM token/notification delivery, Crashlytics upload, Analytics behavior, APNs registration, badge behavior, and tap-through behavior remain explicitly `not claimed`.
...
- The 2026-06-17 Firebase runtime-delivery handoff refresh reports React Native Firebase package current `yes`, Firebase release-services wiring valid `yes`, Android release summary present/valid/current, Android release APK manifest valid, and Firebase runtime delivery validation `not claimed`.
