...
- TypeScript 7 remains blocked on the current tooling baseline because `@typescript-eslint@8.63.0` peers `typescript >=4.8.4 <6.1.0` and `ts-jest@29.4.11` peers `typescript >=4.3 <7`; the 2026-07-11 probe still reports `typescript@7.0.2` as blocked by those three peer ranges. Refresh that evidence with `corepack yarn typescript7:compatibility-probe:audit`.
