...
- `@react-native-community/cli@20.1.3`, `@react-native-community/cli-platform-android@20.1.3`, and `@react-native-community/cli-platform-ios@20.1.3` remain blocked against the current `20.2.0` patch drift until a dedicated RN CLI/tooling branch proves Android assemble, Metro startup, emulator smoke, and iOS static handoff readiness.
- React Navigation package family `@react-navigation/native@7.3.4`, `@react-navigation/stack@7.10.6`, `@react-navigation/bottom-tabs@7.18.3`, and `@react-navigation/devtools@7.1.2` is now the checked navigation patch baseline after peer metadata review, TypeScript/unit/focused tests, Android `devDebug`/`prodDebug` build, and emulator smoke. Current patch drift to `native@7.3.7`, `stack@7.10.10`, `bottom-tabs@7.18.7`, and `devtools@7.1.5` needs a dedicated navigation smoke branch.
- `@sentry/react-native@8.16.0` is the checked release-services baseline. Current patch drift to `8.17.1` needs a dedicated Sentry release-services branch with Android debug/release validation, Sentry prerequisite summaries, and no source-map upload claim without credentials.
- `@typescript-eslint@8.62.0`, `eslint@10.5.0`, `prettier@3.8.4`, and `lint-staged@17.0.8` are now lint/tooling baselines after precommit, lint-staged, TypeScript, and baseline-audit proof. Current patch drift in this cohort still needs a dedicated lint/formatting/tooling branch before changing the baseline snapshot.
