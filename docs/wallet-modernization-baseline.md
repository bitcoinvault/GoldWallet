...
- Latest live RN target snapshot check on `2026-07-11` matched the recorded `2026-07-11` snapshot: `react-native@0.86.0` latest, `0.87.0-rc.0` next, `0.88.0-nightly-20260711-042833e69` nightly, React peer `^19.2.3`, and Node engine `^20.19.4 || ^22.13.0 || ^24.3.0 || >= 25.0.0`.
- Latest live foundation dependency cohort snapshots were refreshed from npm on `2026-07-11`: direct outdated reports 18 entries with 14 known blockers, 4 exotic or git-pinned entries, and 0 review-required entries after the `caniuse-lite@1.0.30001805` Browserslist data resolution refresh. Wallet/crypto reports 15 tracked entries with only the BitcoinVault `bitcoinjs-lib` fork intentionally pinned; storage/network reports 10 current native entries; tooling reports 24 tracked entries with only TypeScript 7 blocked for a dedicated compiler/RN/Metro branch; the TypeScript 7 compatibility probe reports 3 peer-range blockers across `@typescript-eslint/parser`, `@typescript-eslint/eslint-plugin`, and `ts-jest`; and the Android toolchain target remains blocked at AGP `9.2.1` / Gradle `9.6.1` by the RN Gradle plugin `0.86.0` Kotlin metadata path while stable Kotlin stays `2.4.0` and Maven metadata release `2.4.20-Beta1` remains prerelease-only.
- Security resolution baselines after `BEM-37.863` through `BEM-37.872` pin patched transitive versions for `elliptic`, `cipher-base`, `sha.js`, `shell-quote`, `minimist`, Sentry CLI `undici`, `plist`, `simple-plist`, `braces`, `form-data`, `moment`, `qs`, `tmpl`, `tmp`, `joi@17.13.4`, `launch-editor@2.14.1`, `micromatch@4.0.8`, `protobufjs@7.6.5`, `word-wrap@1.2.5`, scoped `**/xcode/uuid@11.1.1`, lockfile `brace-expansion@1.1.16`/`2.1.2`/`5.0.7`, lockfile `js-yaml@3.15.0`/`4.3.0`, lockfile `yaml@1.10.3`/`2.9.0`, and wallet-critical `tiny-secp256k1@2.2.4`, while the lockfile is guarded against vulnerable `ansi-regex`, `base-x@3.0.8`, `brace-expansion@1.1.11`, `minimatch@3.0.4`, `picomatch@2.3.0`, `lodash@4.17.21`, `jws@4.0.0`, `ws@7.5.4`, `tiny-secp256k1@1.1.6`, `joi@17.13.3`, `launch-editor@2.14.0`, `micromatch@4.0.4`, `protobufjs@7.6.1`/`7.6.2`, `word-wrap@1.2.3`, `js-yaml@3.14.1`, `js-yaml@4.1.1`, `yaml@1.10.2`, and `uuid@7.0.3` ranges. `BEM-37.868` keeps Android/RN runtime compatibility through a Metro-only `tiny-secp256k1` shim backed by `@bitcoinerlab/secp256k1`, because the upstream `tiny-secp256k1@2.2.4` browser entry imports WASM. `BEM-37.872` reduces `corepack yarn audit --json --level moderate` from `1` to `0` moderate findings; `corepack yarn audit --json --level low` still reports `2` low findings in the React Native CLI server path (`serve-static` and `send`). `corepack yarn audit --json --level high` now reports `0` critical and `0` high findings, down from `22` critical and `148` high before the security branches.
- The TypeScript 7 compatibility probe was refreshed independently on `2026-07-11`: `typescript@7.0.2` still satisfies the repo Node `v24.16.0`, but remains blocked by `@typescript-eslint/parser`, `@typescript-eslint/eslint-plugin`, and `ts-jest` peer ranges.
