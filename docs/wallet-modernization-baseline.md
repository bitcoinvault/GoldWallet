...
- `corepack yarn ios:mac-validation:handoff:preflight --all-schemes` currently validates Windows-safe iOS static readiness and handoff evidence only: static iOS files are valid, 8 shared schemes are guarded, `ios/Podfile.lock` still has 12 active drift issues, and iOS runtime/archive validation remains not claimed until macOS with Xcode 16.1+ and CocoaPods runs the all-schemes handoff.
