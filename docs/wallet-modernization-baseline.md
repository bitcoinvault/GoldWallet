...
- The latest live npm target snapshot check matched the recorded React Native target snapshot: `react-native@0.86.0` latest, `0.86.0-rc.3` next, `0.87.0-nightly-20260624-5c197fb30` nightly, React peer `^19.2.3`, and Node engine `^20.19.4 || ^22.13.0 || ^24.3.0 || >= 25.0.0`.
...
- `foundation:target:refresh-online` starts with `check:node-runtime-version`, then runs the live npm target snapshot check, direct outdated snapshot audit, generated Babel 8 migration blocker summary, git dependency snapshot audit, wallet/crypto latest snapshot audit, storage/network latest snapshot audit, tooling latest snapshot audit, Android toolchain target audit, BL resolution audit, node-fetch resolution audit, and aggregate foundation target summary validation.
...
- Latest live RN target snapshot check on `2026-06-24` matched the recorded `2026-06-24` snapshot: `react-native@0.86.0` latest, `0.86.0-rc.3` next, `0.87.0-nightly-20260624-5c197fb30` nightly, React peer `^19.2.3`, and Node engine `^20.19.4 || ^22.13.0 || ^24.3.0 || >= 25.0.0`.
- Latest live foundation dependency cohort snapshots were refreshed from npm on `2026-06-24`: direct outdated reports 17 entries with 13 known blockers, 4 exotic or git-pinned entries, and 0 review-required entries after the `react-native-webview@14.0.1`, `react-native-bootsplash@7.3.2`, `react-native-gesture-handler@3.0.2`, `axios@1.18.1`, `semver@7.8.5`, `uuid@14.0.1`, `@typescript-eslint@8.62.0`, and `lint-staged@17.0.8` upgrades; wallet/crypto reports 15 tracked entries with only the BitcoinVault `bitcoinjs-lib` fork intentionally pinned; storage/network reports 10 current native entries; tooling reports all 24 tracked entries current; and the Android toolchain target remains blocked at AGP 9.2.1 / Gradle 9.6.0 by the RN Gradle plugin 0.86.0 Kotlin metadata path while stable Kotlin stays `2.4.0` and Maven metadata release `2.4.20-Beta1` remains prerelease-only.
