...
- `corepack yarn android:dev:check-light` runs the lightweight guard set, QR scanner caller self-check, QR scanner caller inventory guard, and diff whitespace check used before tests in `prepush`
