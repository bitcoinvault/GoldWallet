...
- Security resolution baseline after `BEM-37.863` pins patched transitive versions for `elliptic`, `cipher-base`, `sha.js`, `shell-quote`, `minimist`, Sentry CLI `undici`, `plist`, and `simple-plist`; `corepack yarn audit --json --level high` now reports `0` critical findings, down from `22` before the branch. Remaining high findings require separate owner-path branches, especially wallet-critical `tiny-secp256k1`.
