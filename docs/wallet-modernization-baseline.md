...
- Security resolution baselines after `BEM-37.863` and `BEM-37.864` pin patched transitive versions for `elliptic`, `cipher-base`, `sha.js`, `shell-quote`, `minimist`, Sentry CLI `undici`, `plist`, `simple-plist`, `braces`, `form-data`, `moment`, `qs`, and `tmpl`, while the lockfile is guarded against vulnerable `ansi-regex` ranges. `corepack yarn audit --json --level high` now reports `0` critical and `106` high findings, down from `22` critical and `148` high before the security branches. Remaining high findings require separate owner-path branches, especially wallet-critical `tiny-secp256k1` and `base-x`.
