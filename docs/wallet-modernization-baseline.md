...
- `@react-native-firebase/*` is now on the checked latest `26.1.0` package family; Firebase release delivery remains a runtime validation item, not a package-version blocker. RN Firebase 26.1 introduces automatic Firebase SPM resolution on RN >= 0.75, so the iOS Podfile explicitly keeps CocoaPods until the SPM archive path is validated on macOS/Xcode. The selected Firebase Apple SDK `12.17.0` raises the effective Xcode floor to `26.2`, and the macOS prerequisite audit now verifies that stricter dependency requirement instead of relying only on React Native's `16.1` minimum.
- `react-native-camera` was replaced by `react-native-camera-kit@18.0.0` in the dedicated QR scanner migration stream.
- Navigation/layout packages are on the RN `0.86.2` checkpoint versions checked on 2026-07-31: `@react-navigation/native@7.3.14`, `@react-navigation/stack@7.10.17`, `@react-navigation/bottom-tabs@7.18.14`, `@react-navigation/devtools@7.1.11`, `react-native-gesture-handler@3.1.0`, `react-native-screens@4.26.2`, and `react-native-safe-area-context@5.8.0`; future bumps should stay tied to navigation smoke validation.
...
- Latest local Android release build evidence on `2026-08-10` from `BEM-37.964` records release-input fingerprint `62a0ca660cde438d42872e610065667b588d690091df362da9209c87a49ce3bb` across `513` files. All four release APKs retain the Keychain-only backend and exclude App Center config, resource symbols, and classes. Full `prodRelease` emulator smoke, create-wallet persistence, and public watch-only import persistence pass; Sentry upload and funded transaction validation remain unclaimed.
...
- `corepack yarn sentry:release:validation:production:preflight` now verifies live access to the fixed production Android/iOS projects and runs the upload-disabled handoff through the authenticated managed CLI. `@sentry/react-native@8.22.0`, direct `@sentry/cli@3.6.2`, all three ignored properties files, the four-variant Android release matrix, production smoke, create-wallet persistence, and public watch-only import persistence are ready. The handoff is blocked only by iOS/macOS validation: this Windows host cannot refresh the 12 stale Podfile entries or run Xcode/CocoaPods archive, source-map, and dSYM validation. Production Sentry upload remains explicitly unclaimed. Latest readiness refresh: `BEM-37.964` on 2026-08-10.
- `corepack yarn ios:static:verify` currently validates Windows-safe iOS static readiness and handoff evidence only: static iOS files are valid, 8 shared schemes are guarded, the release-readiness and Podfile refresh-plan artifacts enforce Firebase's effective Xcode `26.2` floor, the generated iOS validation handoff summary is checked as a local artifact, and the executable handoff fails before `pod install` unless macOS, Xcode `26.2+`, and CocoaPods are available. `ios/Podfile.lock` still has 12 active drift issues, and iOS runtime/archive validation remains not claimed until the macOS all-schemes handoff passes. Latest requirement refresh: `BEM-37.961` on 2026-08-10.
