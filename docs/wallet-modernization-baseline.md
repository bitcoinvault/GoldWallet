...
- Latest live foundation dependency cohort snapshots were refreshed from npm on `2026-06-23`: direct outdated reports 17 entries with 13 known blockers, 4 exotic or git-pinned entries, and 0 review-required entries after the `react-native-webview@14.0.1`, `react-native-bootsplash@7.3.2`, `react-native-gesture-handler@3.0.2`, `axios@1.18.1`, `semver@7.8.5`, `uuid@14.0.1`, `@typescript-eslint@8.62.0`, and `lint-staged@17.0.8` upgrades; wallet/crypto reports 15 tracked entries with only the BitcoinVault `bitcoinjs-lib` fork intentionally pinned; storage/network reports 10 current native entries; tooling reports all 24 tracked entries current; and the Android toolchain target remains blocked at AGP 9.2.1 / Gradle 9.6.0 by the RN Gradle plugin 0.86.0 Kotlin metadata path.
