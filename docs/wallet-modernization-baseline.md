...
- Security resolution baselines after `BEM-37.863` through `BEM-37.867` pin patched transitive versions for `elliptic`, `cipher-base`, `sha.js`, `shell-quote`, `minimist`, Sentry CLI `undici`, `plist`, `simple-plist`, `braces`, `form-data`, `moment`, `qs`, `tmpl`, and `tmp`, while the lockfile is guarded against vulnerable `ansi-regex`, `base-x@3.0.8`, `minimatch@3.0.4`, `picomatch@2.3.0`, `lodash@4.17.21`, `jws@4.0.0`, and `ws@7.5.4` ranges. `corepack yarn audit --json --level high` now reports `0` critical and `4` high findings, down from `22` critical and `148` high before the security branches. Remaining high findings are wallet-critical `tiny-secp256k1` and require a dedicated BTCV fork compatibility branch.
