...
- Security resolution baselines after `BEM-37.863` through `BEM-37.868` pin patched transitive versions for `elliptic`, `cipher-base`, `sha.js`, `shell-quote`, `minimist`, Sentry CLI `undici`, `plist`, `simple-plist`, `braces`, `form-data`, `moment`, `qs`, `tmpl`, `tmp`, and wallet-critical `tiny-secp256k1@2.2.4`, while the lockfile is guarded against vulnerable `ansi-regex`, `base-x@3.0.8`, `minimatch@3.0.4`, `picomatch@2.3.0`, `lodash@4.17.21`, `jws@4.0.0`, `ws@7.5.4`, and `tiny-secp256k1@1.1.6` ranges. `BEM-37.868` keeps Android/RN runtime compatibility through a Metro-only `tiny-secp256k1` shim backed by `@bitcoinerlab/secp256k1`, because the upstream `tiny-secp256k1@2.2.4` browser entry imports WASM. `corepack yarn audit --json --level high` now reports `0` critical and `0` high findings, down from `22` critical and `148` high before the security branches.
