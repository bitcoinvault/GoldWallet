...
- Latest live foundation dependency cohort snapshots were refreshed from npm on `2026-07-11`: direct outdated reports 19 entries with 15 known blockers, 4 exotic or git-pinned entries, and 0 review-required entries; the extra known blocker is `caniuse-lite@1.0.30001805` drift against the pinned `1.0.30001803` Browserslist data resolution, which still needs a dedicated lockfile/bundle-transform branch before moving. Wallet/crypto reports 15 tracked entries with only the BitcoinVault `bitcoinjs-lib` fork intentionally pinned; storage/network reports 10 current native entries; tooling reports 24 tracked entries with only TypeScript 7 blocked for a dedicated compiler/RN/Metro branch; the TypeScript 7 compatibility probe reports 3 peer-range blockers across `@typescript-eslint/parser`, `@typescript-eslint/eslint-plugin`, and `ts-jest`; and the Android toolchain target remains blocked at AGP `9.2.1` / Gradle `9.6.1` by the RN Gradle plugin `0.86.0` Kotlin metadata path while stable Kotlin stays `2.4.0` and Maven metadata release `2.4.20-Beta1` remains prerelease-only.
