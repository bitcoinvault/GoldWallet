...
- Sentry Android runtime: GoldWallet breadcrumbs now leave timestamp assignment to `@sentry/react-native@8.22.0`; the `prodRelease` API 36/16 KB smoke no longer emits the previous timestamp-deserialization error, and all Android smoke flows fail on its native `RNSentry` signature
...
- Navigation/layout packages remain on the versions validated with the RN `0.87.0` foundation: `@react-navigation/native@7.3.16`, `@react-navigation/stack@7.10.22`, `@react-navigation/bottom-tabs@7.18.16`, `@react-navigation/devtools@7.1.12`, `react-native-gesture-handler@3.1.0`, `react-native-screens@4.27.0`, and `react-native-safe-area-context@5.9.0`; future bumps should stay tied to navigation smoke validation.
...
- `react-native-prompt-android` remains wallet-critical for encrypted-storage startup. Its versioned patch replaces the legacy support AlertDialog import and AppCompat dependency with AndroidX, retains pending arguments and callback across the pre-resume lifecycle and Activity replacement, resolves the current FragmentManager only when displaying, and clears lifecycle state during module invalidation so encrypted-wallet startup cannot remain on the splash screen or retain a stale Activity; global Jetifier is retired.
...
- `postinstall` runs `patch-package`, `rn-nodeify`, and the repository shim repair. Jetifier is deliberately absent; skipping scripts requires running the remaining patch/shim steps manually before Android build or smoke testing.
...
- `corepack yarn ios:static:verify` currently validates Windows-safe iOS static readiness and handoff evidence only: static iOS files are valid, 8 shared schemes are guarded, the first-party historical Keychain bridge belongs to all 4 app target source phases, and the removed third-party pod no longer appears in `ios/Podfile.lock`. The lockfile still has 13 active version-drift issues. iOS migration runtime, archive delivery, and fallback removal remain unclaimed until the macOS all-schemes handoff and a historical-data upgrade test pass. Latest requirement refresh: `BEM-37.973` on 2026-08-11.
...
- Latest live foundation dependency cohort snapshots were refreshed from npm through `2026-08-11`. Wallet/crypto keeps only the BitcoinVault `bitcoinjs-lib` fork intentionally pinned; storage/network reports 9 current entries after `BEM-37.932`; tooling reports 24 tracked entries after the global Jetifier retirement, with `jsdom@30.0.1`, `@types/jsdom@30.0.0`, `@typescript-eslint@8.67.0`, `eslint@10.8.1`, and `lint-staged@17.3.0`. TypeScript 7 remains assigned to a dedicated compiler/RN/Metro branch because of its recorded `@typescript-eslint` and `ts-jest` peer blockers. The RN `0.87.0` foundation adopts AGP `9.2.1` / Gradle `9.4.1` and aligns Kotlin/KSP at `2.2.10`; newer Android toolchain movement remains a separate compatibility branch.
