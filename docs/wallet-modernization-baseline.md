...
- Latest live foundation dependency cohort snapshots were refreshed from npm on `2026-06-23`: direct outdated reports 23 entries with 19 known blockers, 4 exotic or git-pinned entries, and 0 review-required entries after the `react-native-webview@14.0.1`, `react-native-bootsplash@7.3.2`, `react-native-gesture-handler@3.0.2`, and `axios@1.18.1` upgrades; wallet/crypto reports 15 tracked entries with only the BitcoinVault `bitcoinjs-lib` fork intentionally pinned; storage/network reports 10 current native entries; tooling reports 21 current entries plus `@typescript-eslint` and `lint-staged` patch drift blocked to a dedicated lint/tooling branch; and the Android toolchain target remains blocked at AGP 9.2.1 / Gradle 9.6.0 by the RN Gradle plugin 0.86.0 Kotlin metadata path.
