...
- `react-native-prompt-android` remains wallet-critical for encrypted-storage startup, but its versioned patch now replaces the legacy support AlertDialog import and AppCompat dependency with AndroidX before compilation; global Jetifier is retired.
