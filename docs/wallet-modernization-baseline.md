...
- The latest live npm target snapshot check matched the recorded React Native target snapshot: `react-native@0.86.0` latest, `0.86.0-rc.3` next, `0.87.0-nightly-20260623-dc4d5e8ad` nightly, React peer `^19.2.3`, and Node engine `^20.19.4 || ^22.13.0 || ^24.3.0 || >= 25.0.0`.
...
- `@react-native-firebase/*` is now on the checked latest `24.1.1` package family; Firebase release delivery remains a runtime validation item, not a package-version blocker.
- `react-native-camera` was replaced by `react-native-camera-kit@18.0.0` in the dedicated QR scanner migration stream.
- Navigation/layout packages are on the RN `0.86.0` checkpoint versions: `@react-navigation/native@7.3.3`, `@react-navigation/stack@7.10.5`, `@react-navigation/bottom-tabs@7.18.2`, `@react-navigation/devtools@7.1.1`, `react-native-gesture-handler@3.0.2`, `react-native-screens@4.25.2`, and `react-native-safe-area-context@5.8.0`; future bumps should stay tied to navigation smoke validation.
...
- `foundation:target:refresh-online` starts with `check:node-runtime-version`, then runs the live npm target snapshot check, direct outdated snapshot audit, Babel 8 migration blocker probe, git dependency snapshot audit, wallet/crypto latest snapshot audit, storage/network latest snapshot audit, tooling latest snapshot audit, Android toolchain target audit, BL resolution audit, node-fetch resolution audit, and aggregate foundation target summary validation.
...
- Latest live RN target snapshot check on `2026-06-23` matched the recorded `2026-06-23` snapshot: `react-native@0.86.0` latest, `0.86.0-rc.3` next, `0.87.0-nightly-20260623-dc4d5e8ad` nightly, React peer `^19.2.3`, and Node engine `^20.19.4 || ^22.13.0 || ^24.3.0 || >= 25.0.0`.
- Latest live foundation dependency cohort snapshots were refreshed from npm on `2026-06-23`: direct outdated reports 20 entries with 16 known blockers, 4 exotic or git-pinned entries, and 0 review-required entries after the `react-native-webview@14.0.1`, `react-native-bootsplash@7.3.2`, `react-native-gesture-handler@3.0.2`, `axios@1.18.1`, `semver@7.8.5`, and `uuid@14.0.1` upgrades; wallet/crypto reports 15 tracked entries with only the BitcoinVault `bitcoinjs-lib` fork intentionally pinned; storage/network reports 10 current native entries; tooling reports 21 current entries plus `@typescript-eslint` and `lint-staged` patch drift blocked to a dedicated lint/tooling branch; and the Android toolchain target remains blocked at AGP 9.2.1 / Gradle 9.6.0 by the RN Gradle plugin 0.86.0 Kotlin metadata path.
