...
- The latest live npm target snapshot check matched the recorded React Native target snapshot: `react-native@0.86.2` latest, `0.87.0-rc.3` next, a valid prerelease nightly tag, React peer `^19.2.3`, and Node engine `^20.19.4 || ^22.13.0 || ^24.3.0 || >= 25.0.0`.
...
- `corepack yarn sentry:release:validation:preflight` currently passes only as a controlled `not ready` source-map readiness check: `@sentry/react-native@8.19.0` and deduplicated root `@sentry/cli@3.6.1` are current, the Android resolver is verified against the direct package, and Sentry Android warning plus RN bundle-task compatibility summaries are valid. `SENTRY_AUTH_TOKEN`, root/Android/iOS `sentry.properties`, full release smoke/create-wallet proof, and macOS iOS archive validation are still required before release upload can be claimed. Latest Sentry cohort evidence refresh: `BEM-37.921` on 2026-07-21.
