...
- Android lightweight check runs the Android warning baseline guard, Android warning artifact guard, camera usage self-check/inventory guard, QR scanner caller self-check/inventory guard, Sentry usage self-check/inventory guard, native module inventory self-check/inventory guard, RN nodeify shim self-check/inventory guard, TypeScript check, and diff whitespace check.
