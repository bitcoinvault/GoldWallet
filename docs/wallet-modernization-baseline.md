...
- Latest live RN target snapshot check on `2026-07-10` matched the recorded `2026-07-10` snapshot: `react-native@0.86.0` latest, `0.87.0-rc.0` next, `0.88.0-nightly-20260710-102fde7b6` nightly, React peer `^19.2.3`, and Node engine `^20.19.4 || ^22.13.0 || ^24.3.0 || >= 25.0.0`.
- Latest live foundation dependency cohort snapshots were refreshed from npm on `2026-07-10`: direct outdated reports 18 entries with 14 known blockers, 4 exotic or git-pinned entries, and 0 review-required entries; wallet/crypto reports 15 tracked entries with only the BitcoinVault `bitcoinjs-lib` fork intentionally pinned; storage/network reports 10 current native entries; tooling reports 24 tracked entries with only TypeScript 7 blocked for a dedicated compiler/RN/Metro branch; the TypeScript 7 compatibility probe reports 3 peer-range blockers across `@typescript-eslint/parser`, `@typescript-eslint/eslint-plugin`, and `ts-jest`; and the Android toolchain target remains blocked at AGP `9.2.1` / Gradle `9.6.1` by the RN Gradle plugin `0.86.0` Kotlin metadata path while stable Kotlin stays `2.4.0` and Maven metadata release `2.4.20-Beta1` remains prerelease-only.
