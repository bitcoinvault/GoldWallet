...
- `bl` is on checked highest compatible `6.1.6` after `BEM-37.321`; latest `7.0.2` is blocked because it is ESM/export-only while `levelup` and `ora` still require `bl` through CommonJS.
...
- ESLint is on checked latest `10.4.1` after `BEM-37.304`; `eslint.config.mjs` bridges the existing `.eslintrc` baseline through `FlatCompat`, `.eslintignore` is removed, and the baseline remains `35346` errors / `0` warnings.
- TypeScript ESLint parser/plugin tooling is on checked latest `8.60.0` after `BEM-37.293`; it is verified through the ESLint 10 flat-config bridge because `@typescript-eslint@8.60.0` supports `eslint ^8.57.0 || ^9.0.0 || ^10.0.0`.
