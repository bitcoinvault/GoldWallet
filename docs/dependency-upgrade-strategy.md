...
- Small tooling patch cohort is refreshed through `BEM-37.924`: `@types/react@19.2.17`, `@typescript-eslint/eslint-plugin@8.65.0`, `@typescript-eslint/parser@8.65.0`, `eslint@10.7.0`, `@eslint/js@10.0.1`, `@eslint/eslintrc@3.3.6`, `@eslint/compat@2.1.0`, `jiti@2.7.0`, `detox@20.51.4`, `lint-staged@17.1.0`, `prettier@3.9.6`, `semver@7.8.5`, `caniuse-lite@1.0.30001806`, and `junit-report-merger@9.0.4`.
...
- `lint-staged` is on checked latest `17.1.0` after BEM-37.924 on the Node 24 tooling baseline; it requires Node `>=22.22.1`, so run hooks and validation with the `.nvmrc` Node runtime. The `precommit`, `prepush`, `android:dev:check-light`, `rn:baseline:preflight`, and `rn:baseline:preflight:online` scripts intentionally run `check:node-runtime-version` first so Git hooks and direct validation entrypoints fail fast unless the active shell matches `.nvmrc`; `precommit` then runs `lint-staged:tooling:audit` before `lint-staged` instead of relying on package-manager engine behavior.
