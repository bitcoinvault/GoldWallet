...
- React Navigation is on checked latest v7 patch baselines after `BEM-37.822`: `@react-navigation/native@7.3.7`, `@react-navigation/stack@7.10.10`, `@react-navigation/bottom-tabs@7.18.7`, and `@react-navigation/devtools@7.1.5`. `react-native-screens@4.25.2`, `react-native-safe-area-context@5.8.0`, and `react-native-gesture-handler@3.0.2` were already current latest at branch time. Future navigation drift should stay in a dedicated navigation/smoke branch.
...
- Small tooling patch cohort is refreshed through `BEM-37.820`: `@types/react@19.2.17`, `@typescript-eslint/eslint-plugin@8.62.1`, `@typescript-eslint/parser@8.62.1`, `eslint@10.6.0`, `detox@20.51.4`, `prettier@3.9.4`, `semver@7.8.5`, and `junit-report-merger@9.0.4`.
...
- Prettier tooling is on checked latest Prettier 3 line after `BEM-37.820`: `prettier@3.9.4`, `eslint-plugin-prettier@5.5.6`, and `eslint-config-prettier@10.1.8`; no mass formatting was performed, and the existing lint baseline remains tracked separately.
...
- TypeScript ESLint parser/plugin tooling is on checked latest `8.62.1` after `BEM-37.820`; it is verified through the ESLint 10 flat-config bridge because `@typescript-eslint@8.62.1` supports `eslint ^8.57.0 || ^9.0.0 || ^10.0.0`.
