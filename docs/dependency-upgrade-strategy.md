...
- Security transitive resolutions are guarded after `BEM-37.863`: `elliptic@6.6.1`, `cipher-base@1.0.7`, `sha.js@2.4.12`, `shell-quote@1.10.0`, `minimist@1.2.8`, Sentry CLI `undici@6.27.0`, `plist@3.1.1`, and `simple-plist@1.3.1`. The branch reduced `corepack yarn audit --json --level high` critical findings from `22` to `0`; remaining high findings need separate owner-path branches rather than unscoped overrides.
