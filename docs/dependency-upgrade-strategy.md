...
- The current Android baseline stays on AGP `8.13.2`, Gradle `8.13`, Kotlin `2.1.20`, SDK `36`, and JDK `17`; use `corepack yarn android:toolchain-target:audit` before another Android toolchain branch. Live metadata on 2026-06-17 showed AGP `9.2.1`, Gradle `9.5.1`, and Kotlin `2.4.0`, but the AGP 9 path is blocked until the React Native Gradle plugin can compile against Gradle 9's embedded Kotlin metadata.
...
- Small tooling patch cohort is refreshed through `BEM-37.764`: `@types/react@19.2.17`, `@typescript-eslint/eslint-plugin@8.61.1`, `@typescript-eslint/parser@8.61.1`, `eslint@10.5.0`, `detox@20.51.4`, `prettier@3.8.4`, `semver@7.8.5`, and `junit-report-merger@9.0.4`.
- `lint-staged` is on checked latest `17.0.7` after the Node 24 tooling baseline; it requires Node `>=22.22.1`, so run hooks and validation with the `.nvmrc` Node runtime. The `precommit`, `prepush`, `android:dev:check-light`, `rn:baseline:preflight`, and `rn:baseline:preflight:online` scripts intentionally run `check:node-runtime-version` first so Git hooks and direct validation entrypoints fail fast unless the active shell matches `.nvmrc`; `precommit` then runs `lint-staged:tooling:audit` before `lint-staged` instead of relying on package-manager engine behavior.
...
- TypeScript ESLint parser/plugin tooling is on checked latest `8.61.1` after `BEM-37.681`; it is verified through the ESLint 10 flat-config bridge because `@typescript-eslint@8.61.1` supports `eslint ^8.57.0 || ^9.0.0 || ^10.0.0`.
