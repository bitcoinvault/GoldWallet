...
This project should not upgrade dependencies one package at a time unless the package is isolated and low risk. The app is now on the RN `0.87.0` / React `19.2.3` foundation checkpoint; RN `0.87.0` currently matches npm `latest`, while React `19.2.8` is intentionally blocked by the RN renderer exact-version constraint on this baseline. The historical npm `next` RC tag remains planning evidence rather than the default wallet target. The upgrade path should therefore keep moving by layers, keep live snapshot checks at each foundation branch, and avoid returning to package-by-package churn.
...
- The current RN target snapshot records `react-native@0.87.0` with React peer `^19.2.3`, matching the installed RN `0.87.0` checkpoint and React `19.2.3`; do not take React `19.2.8` separately because the app has already recorded an emulator-startup mismatch against `react-native-renderer@19.2.3`.
...
- The latest React type patch is refreshed through `BEM-37.979`: `@types/react@19.2.18`. The broader lint/tooling cohort remains `@typescript-eslint/eslint-plugin@8.67.0`, `@typescript-eslint/parser@8.67.0`, `eslint@10.8.1`, `@eslint/js@10.0.1`, `@eslint/eslintrc@3.3.6`, `@eslint/compat@2.1.0`, `jiti@2.7.0`, `detox@20.51.4`, `lint-staged@17.3.0`, `prettier@3.9.6`, `ts-jest@29.4.12`, `jsdom@30.0.1`, `semver@7.8.5`, `caniuse-lite@1.0.30001806`, and `junit-report-merger@9.0.4`.
- Security transitive baselines are guarded after `BEM-37.863` through `BEM-37.972`: `elliptic@6.6.1`, `cipher-base@1.0.7`, `sha.js@2.4.12`, `shell-quote@1.10.0`, `minimist@1.2.8`, Sentry CLI `undici@8.10.0`, jsdom-owned `undici@8.9.0`, `plist@3.1.1`, `simple-plist@1.3.1`, `braces@3.0.3`, `form-data@4.0.6`, `moment@2.30.1`, `qs@6.15.3`, `tmpl@1.0.5`, `tmp@0.2.7`, `joi@18.2.3`, `launch-editor@2.14.1`, `micromatch@4.0.8`, `protobufjs@8.7.2`, `word-wrap@1.2.5`, scoped `**/xcode/uuid@14.0.1`, `serve-static@2.2.1`, `send@1.2.1`, and the guarded lockfile-only versions recorded by `check:security-resolution-baselines`. `BEM-37.971` validates the Firebase/Firestore proto-loader major override, and `BEM-37.972` validates owner-context transport resolution, CommonJS transport APIs, local HTTP fetch/response streaming, the active CLI binary, and actual CLI version output. The last successful `corepack yarn audit --level low` reported `0 vulnerabilities found`; on 2026-07-15 the Yarn Classic registry audit endpoint returned HTTP 410, so the current branch relies on the guarded resolution/lockfile baseline and records the audit-service outage explicitly. The security branches reduced `corepack yarn audit --json --level high` from `22` critical / `148` high to `0` critical / `0` high.
...
- The Sentry release-services baseline is current after BEM-37.972: `@sentry/react-native@8.22.0`, direct `@sentry/cli@3.6.2`, and Sentry-scoped `undici@8.10.0`. The SDK upgrade retains the versioned source-map helper patch because upstream `8.22.0` still ships two `process.exist(1)` typos. Future work in this owner path must preserve full release/source-map generation, production emulator smoke, and secret-safe credential gating.
...
- The complete React Native framework cohort is now aligned on stable `0.87.0`: runtime, Babel preset, codegen, Gradle plugin, Jest preset, Metro config, and TypeScript config. Live direct-outdated evidence must omit these packages while `0.87.0` remains current and must reintroduce the whole cohort together when a newer stable RN line becomes the next dedicated runtime milestone.
...
- Flipper debug tooling is removed after `BEM-37.324` instead of bumped: `react-native-flipper@0.273.0` remains npm `latest` but only peers React 16/17/18, while the current RN `0.87.0` baseline uses React `19.2.3`.
