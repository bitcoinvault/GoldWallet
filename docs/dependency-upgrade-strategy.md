...
This project should not upgrade dependencies one package at a time unless the package is isolated and low risk. The app is now on the RN `0.86.2` / React `19.2.3` foundation checkpoint; RN `0.86.2` currently matches npm `latest`, while React `19.2.8` is intentionally blocked by the RN renderer exact-version constraint on this baseline. npm `next` for React Native is an RC line, is classified as `prerelease`, and is not the default wallet target. The upgrade path should therefore keep moving by layers, keep live snapshot checks at each foundation branch, and avoid returning to package-by-package churn.
...
- The current RN target snapshot records `react-native@0.86.2` with React peer `^19.2.3`, matching the installed RN `0.86.2` checkpoint and React `19.2.3`; do not take React `19.2.8` separately because the app has already recorded an emulator-startup mismatch against `react-native-renderer@19.2.3`.
...
- React Navigation is on the checked latest v7 patch baseline as of 2026-08-10: `@react-navigation/native@7.3.16`, `@react-navigation/stack@7.10.22`, `@react-navigation/bottom-tabs@7.18.16`, and `@react-navigation/devtools@7.1.12`. `react-native-screens@4.27.0` and `react-native-safe-area-context@5.8.1` are the checked latest native navigation peers; `react-native-gesture-handler@3.1.0` remains current. Future navigation drift should stay in a dedicated navigation/smoke branch.
...
- TypeScript 7 is tracked by `corepack yarn typescript7:compatibility-probe:audit`; current live metadata reports `typescript@7.0.2`, and it remains blocked until a dedicated compiler/RN/Metro branch moves TypeScript, `@typescript-eslint`, and Jest transform tooling together because `@typescript-eslint@8.67.0` peers `typescript >=4.8.4 <6.1.0` and `ts-jest@29.4.12` peers `typescript >=4.3 <7`. The probe strengthens that decision with an isolated latest-cohort install: normal npm install fails with `ERESOLVE`, and the legacy-peer fallback confirms the same peer ceilings without changing the repo lockfile.
- Babel `8.x` is recorded as blocked after `BEM-37.682` because RN `0.86.2` still ships a Babel preset dependency stack on Babel `7.x` plugins. Treat Babel 8 as a dedicated RN/Metro/Babel migration branch, not as a direct package patch. The validated Babel 7 resolution includes the latest stable `@babel/traverse@7.29.8`; patch movement inside Babel 7 does not relax the major-version blocker. Use `corepack yarn babel8:migration-probe:audit`, `corepack yarn babel8:migration-probe:check-summary`, and `corepack yarn babel8:migration-probe:check` to refresh the generated `local-docs/babel-8-migration-probe-summary.txt` evidence and keep the static blocker evidence aligned. The probe installs the full stable Babel 8 cohort in isolation, selects the highest stable `8.x` traverse release independently of npm's `latest` tag, and then proves the RN preset transform failure, so the blocker is tied to actual package behavior rather than metadata only.
...
- The tooling snapshot also tracks isolated report/E2E/coverage tooling: `jest-junit@17.0.0`, `junit-report-merger@9.0.4`, `babel-plugin-istanbul@8.0.0`, `mailosaur@11.1.1`, `jsdom@30.0.1`, and matching `@types/jsdom@30.0.0`.
- The latest lint/tooling patch cohort is refreshed through `BEM-37.970`: `@types/react@19.2.17`, `@typescript-eslint/eslint-plugin@8.67.0`, `@typescript-eslint/parser@8.67.0`, `eslint@10.8.1`, `@eslint/js@10.0.1`, `@eslint/eslintrc@3.3.6`, `@eslint/compat@2.1.0`, `jiti@2.7.0`, `detox@20.51.4`, `lint-staged@17.3.0`, `prettier@3.9.6`, `ts-jest@29.4.12`, `jsdom@30.0.1`, `semver@7.8.5`, `caniuse-lite@1.0.30001806`, and `junit-report-merger@9.0.4`.
- Security transitive baselines are guarded after `BEM-37.863` through `BEM-37.926`: `elliptic@6.6.1`, `cipher-base@1.0.7`, `sha.js@2.4.12`, `shell-quote@1.10.0`, `minimist@1.2.8`, Sentry CLI `undici@8.8.0`, `plist@3.1.1`, `simple-plist@1.3.1`, `braces@3.0.3`, `form-data@4.0.6`, `moment@2.30.1`, `qs@6.15.3`, `tmpl@1.0.5`, `tmp@0.2.7`, `joi@18.2.3`, `launch-editor@2.14.1`, `micromatch@4.0.8`, `protobufjs@8.7.1`, `word-wrap@1.2.5`, scoped `**/xcode/uuid@14.0.1`, `serve-static@2.2.1`, `send@1.2.1`, lockfile `ansi-regex`, lockfile `base-x@3.0.11`, lockfile `brace-expansion@1.1.16`/`2.1.2`/`5.0.7`, lockfile `js-yaml@3.15.0`/`4.3.0`, lockfile `yaml@1.10.3`/`2.9.0`, lockfile `minimatch@3.1.5`, lockfile `picomatch@2.3.2`/`4.0.4`/`4.0.5`, lockfile `lodash@4.18.1`, lockfile `jws@4.0.1`, lockfile `ws@7.5.11`, and wallet-critical `tiny-secp256k1@2.2.4`. `BEM-37.868` keeps React Native runtime compatibility through a Metro-only `tiny-secp256k1` shim backed by `@bitcoinerlab/secp256k1`, because the upstream browser entry imports WASM. `BEM-37.872` reduced the remaining moderate audit surface from `1` to `0` findings by moving the `react-native-bootsplash > @expo/config-plugins > xcode > uuid` owner path from `uuid@7.0.3` to patched `uuid@11.1.1`; `BEM-37.873` cleared the low audit surface with compatible RN CLI server-path patches; `BEM-37.875` accepted the latest `serve-static@2.2.1` / `send@1.2.1` major owner path after Metro startup, Android build, and emulator no-network smoke proof; `BEM-37.876` accepts latest `uuid@14.0.1` for the `xcode` owner path after xcode parse/generate proof; `BEM-37.877` accepts latest `joi@18.2.3` for the RN CLI config/type owner path after RN CLI config, Android build, and emulator no-network smoke proof; `BEM-37.902` refreshes the Firebase/Firestore proto-loader owner path to latest `protobufjs@8.7.1` after CommonJS/proto-loader API probes, Android build, and emulator no-network smoke proof; `BEM-37.926` refreshes the validated Sentry CLI transport owner path to latest `undici@8.8.0` after CJS API, Sentry CLI, release-service prerequisite, Android build, and emulator no-network smoke proof. The last successful `corepack yarn audit --level low` reported `0 vulnerabilities found`; on 2026-07-15 the Yarn Classic registry audit endpoint returned HTTP 410, so the current branch relies on the guarded resolution/lockfile baseline and records the audit-service outage explicitly. The security branches reduced `corepack yarn audit --json --level high` from `22` critical / `148` high to `0` critical / `0` high.
...
- `lint-staged` is on checked latest `17.3.0` after BEM-37.933 on the Node 24 tooling baseline; it requires Node `>=22.22.1`, so run hooks and validation with the `.nvmrc` Node runtime. The `precommit`, `prepush`, `android:dev:check-light`, `rn:baseline:preflight`, and `rn:baseline:preflight:online` scripts intentionally run `check:node-runtime-version` first so Git hooks and direct validation entrypoints fail fast unless the active shell matches `.nvmrc`; `precommit` then runs `lint-staged:tooling:audit` before `lint-staged` instead of relying on package-manager engine behavior.
- The Sentry release-services baseline is current after BEM-37.960: `@sentry/react-native@8.22.0`, direct `@sentry/cli@3.6.2`, and Sentry-scoped `undici@8.9.0`. The SDK upgrade retains the versioned source-map helper patch because upstream `8.22.0` still ships two `process.exist(1)` typos. Future work in this owner path must preserve full release/source-map generation, production emulator smoke, and secret-safe credential gating.
- Prettier tooling is on checked latest Prettier 3 line after `BEM-37.924`: `prettier@3.9.6`, `eslint-plugin-prettier@5.5.6`, and `eslint-config-prettier@10.1.8`; no mass formatting was performed, and the existing lint baseline remains tracked separately.
- ESLint is on checked latest `10.8.1` after `BEM-37.970`; `eslint.config.mjs` bridges the existing `.eslintrc` baseline through `FlatCompat`, `.eslintignore` is removed, and the current lint baseline remains tracked separately.
- TypeScript ESLint parser/plugin tooling is on checked latest `8.67.0` after `BEM-37.970`; it is verified through the ESLint 10 flat-config bridge because `@typescript-eslint@8.67.0` supports `eslint ^8.57.0 || ^9.0.0 || ^10.0.0` and `typescript >=4.8.4 <6.1.0`.
- The Firebase/Firestore `@grpc/proto-loader` owner path is on checked latest `protobufjs@8.7.2` after `BEM-37.971`. Because the installed loader still declares `protobufjs@^7.2.5`, `check:protobufjs-owner-path` guards the deliberate major resolution, the complete Firebase owner chain, CommonJS API, message encode/decode, and `loadSync` service definitions.
- Live direct-outdated metadata now exposes six React Native framework package latest tags at `0.87.0`, while the React Native runtime latest tag remains `0.86.2` and `next` is `0.87.0-rc.4`. Keep `@react-native/babel-preset`, codegen, Gradle plugin, Jest preset, Metro config, and TypeScript config aligned with the runtime in one dedicated RN 0.87 branch; do not patch these framework packages independently.
...
- Flipper debug tooling is removed after `BEM-37.324` instead of bumped: `react-native-flipper@0.273.0` remains npm `latest` but only peers React 16/17/18, while the current RN `0.86.2` baseline uses React `19.2.3`.
