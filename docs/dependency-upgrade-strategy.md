...
- When network access is available for a React Native foundation branch, use `corepack yarn rn:baseline:preflight:online` so the live RN target snapshot, direct outdated snapshot, git dependency snapshot, wallet/crypto latest snapshot, storage/network latest snapshot, tooling latest snapshot, Android toolchain target, BL resolution, and node-fetch resolution summaries are refreshed, validated through `foundation:target:check-summaries`, and then followed by the offline baseline gate.
...
- The current Android baseline stays on AGP `8.13.2`, Gradle `8.13`, Kotlin `2.1.20`, SDK `36`, and JDK `17`; use `corepack yarn android:toolchain-target:audit` before another Android toolchain branch. Live metadata on 2026-06-10 showed AGP `9.2.1`, Gradle `9.5.1`, and Kotlin `2.4.0`, but the AGP 9 path is blocked until the React Native Gradle plugin can compile against Gradle 9's embedded Kotlin metadata.
...
- `lint-staged` is on checked latest `17.0.7` after the Node 24 tooling baseline; it requires Node `>=22.22.1`, so run hooks and validation with the `.nvmrc` Node runtime. The `precommit` and `prepush` scripts intentionally run `check:node-runtime-version` first so Git hooks fail fast unless the active shell matches `.nvmrc`; `precommit` then runs `lint-staged:tooling:audit` before `lint-staged` instead of relying on package-manager engine behavior.
