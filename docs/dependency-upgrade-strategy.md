...
- `lint-staged` is on checked latest `17.0.7` after the Node 24 tooling baseline; it requires Node `>=22.22.1`, so run hooks and validation with the `.nvmrc` Node runtime. The `precommit` script intentionally runs `lint-staged:tooling:audit` before `lint-staged` so commits fail fast under an older Node shell instead of relying on package-manager engine behavior.
