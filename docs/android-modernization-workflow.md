...
`check:store-metadata-readiness-guard` verifies the store metadata readiness guard fixtures. `check:store-metadata-readiness` verifies that iOS Fastlane metadata, Android Fastlane metadata, Play Store icon source, and store/rebrand documentation still capture the pre-rebrand baseline.
