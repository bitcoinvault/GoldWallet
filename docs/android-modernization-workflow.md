...
The TypeScript 7 compatibility probe checks live `typescript@latest` against the current lint and Jest transform peer ranges. On the current baseline it keeps `typescript@6.0.3` pinned because `@typescript-eslint@8.63.0` does not peer beyond `<6.1.0` and `ts-jest@29.4.11` does not peer beyond `<7`.
