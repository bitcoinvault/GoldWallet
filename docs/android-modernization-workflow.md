...
Latest local release build evidence refresh: on 2026-06-17, `feature/bem-37-735-sentry-release-evidence-refresh` rebuilt `devRelease`, `stageRelease`, `prodRelease`, and `betaRelease` with JDK 17 and validated the generated summary artifact plus APK manifests after `sentry:release:prereq-audit` reported a stale release-input fingerprint. Latest release-smoke proof: on 2026-06-17, `feature/bem-37-725-android-release-validation-refresh` installed a locally signed `devRelease` smoke APK on `emulator-5554` without Metro, completed first-run terms/PIN/transaction-password setup, validated empty-dashboard CTA navigation, tab navigation, QR scanner, Settings Terms WebView, then ran release create-wallet smoke through standard-wallet mnemonic backup and default 3-key vault public-key integration screens. Earlier `feature/bem-37-715-android-release-evidence-refresh`, `feature/bem-37-590-android-release-evidence-refresh`, `feature/bem-37-645-android-release-evidence-refresh`, `feature/bem-37-646-android-release-smoke-refresh`, and `feature/bem-37-671-android-release-evidence-refresh` established the release-input fingerprint and aggregate release-services evidence pattern after the RN `0.86.0` foundation. Keep future release proof branches on the same summary/checker flow so APK evidence stays local while the tracked log records the milestone.
