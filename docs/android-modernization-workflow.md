...
Use `android:dev:release:network-blocker:audit` after a release-smoke run reaches `No network`. It writes `local-docs/android-release-network-blocker-summary.txt` by combining the failed full release-smoke summary, the reduced `android-smoke-dev-release-no-network` summary, and `local-docs/android-smoke-dev-release.log`. `android:dev:release:network-blocker:check-summary` requires the signed `devRelease` no-network proof, `SSLHandshakeException`, and `CertificateExpiredException` evidence before classifying the blocker as `blocked-by-electrum-certificate-expired`. This audit is not a substitute for `release-services:check-summaries`; it exists to prove the external dev/testnet Electrum TLS blocker while keeping full dashboard/create-wallet release validation red until the certificate is renewed and the smoke flow is rerun.
