...
Latest Sentry release preflight evidence refresh: on 2026-06-24, `feature/bem-37-791-sentry-release-readiness-refresh` revalidated `@sentry/react-native@8.15.1` and `@sentry/cli@3.5.1` as current npm latest targets, refreshed `dev`, `stage`, `prod`, and `beta` Android release build/source-map evidence with JDK 17 and `SENTRY_DISABLE_AUTO_UPLOAD=true`, and re-ran release smoke on `emulator-5554`. Full release smoke and release create-wallet smoke remain blocked by the dev/testnet `No network` screen, but the reduced `android-smoke-dev-release-no-network` proof is valid for signed `devRelease` install, launch, onboarding, expected `No network` UI, and no fatal/runtime logcat findings. `sentry:release:validation:preflight` may pass in that controlled `not ready` state without running the final aggregate `release-services:check-summaries` gate; it still keeps Sentry source-map upload unclaimed until `SENTRY_AUTH_TOKEN`, root/Android/iOS `sentry.properties`, macOS/Xcode/CocoaPods iOS validation, and full release smoke/create-wallet evidence are available.
...
Latest iOS static readiness evidence refresh: on 2026-06-24, `feature/bem-37-787-ios-handoff-refresh` re-ran static release readiness, macOS prerequisite, Podfile.lock refresh-plan, preflight dry-run, and validation handoff summary checks for RN `0.86.0` after the Firebase `25.0.1` package-family upgrade. Static iOS files remain valid, eight guarded schemes are tracked, removed-pod references remain at `0`, Sentry bundle/source-map and dSYM phases remain present, and runtime/archive validation remains not claimed on Windows because macOS with Xcode `16.1+`, CocoaPods, and a refreshed `ios/Podfile.lock` are still required. The current `ios/Podfile.lock` drift count is 12 active package-vs-pod entries, including `RNFBApp 12.7.5` versus `@react-native-firebase/app 25.0.1`, and the combined iOS handoff summary remains `Implementation ready: no` until those macOS-only steps pass. Earlier `feature/bem-37-769-ios-static-readiness-refresh` established the same Windows-safe iOS handoff pattern before the Firebase `25.0.1` refresh.
