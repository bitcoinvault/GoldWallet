...
`codepush:release:path-audit` records CodePush non-dev runtime/native/env wiring, per-env deployment-key readiness, live npm latest metadata, App Center CodePush retirement state, upstream archive/New Architecture support status, whether the latest local Android release summary artifact is present and valid, whether that release summary covers the current release inputs, whether Android release build evidence is ready, and whether CodePush update validation is claimed. The expected local state is that APK/bundle generation can be proven by Android release summaries while CodePush update validation remains unclaimed until non-empty deployment keys and the beta strategy are available. `codepush:migration:readiness-audit` keeps the remove-or-replace decision visible as a guarded release-service summary. `codepush:removal-readiness:audit` inventories the exact runtime, native, env, and plist surfaces that must be deleted or replaced before CodePush can be removed, and it keeps `Safe to remove now: no` until the remove-or-replace decision is explicit. Because App Center CodePush was retired on 2025-03-31 and the Microsoft upstream is archived, CodePush is now tracked as a migration/removal workstream instead of a normal dependency-refresh target.
...
`firebase:runtime:delivery:handoff:dry-run` prints the Firebase-specific runtime-delivery prerequisite sequence. The executable sequence optionally refreshes Android release APK evidence with Sentry auto-upload disabled, refreshes Firebase release-services and push notification bridge summaries, validates the aggregate release-services summaries, and then verifies that local Firebase/push prerequisites are ready while real FCM token/notification delivery, Crashlytics upload, and Analytics behavior remain explicitly not claimed. `check:firebase-runtime-delivery-handoff-guard` keeps this command order and not-claimed boundary from drifting. Use `--skip-android-release` only when current Android release evidence is already fresh.
...
`ios:mac-validation:handoff:dry-run` prints the exact macOS execution sequence for iOS handoff without claiming runtime validation on Windows: prerequisite audit/check, `pod install`, iOS release readiness audit/check, selected Xcode simulator build, and post-build readiness audit/check. `check:ios-mac-validation-handoff-guard` keeps the known shared scheme/configuration matrix and handoff command order from drifting before the workflow is executed on macOS, and it is part of `rn:baseline:preflight`.

`release-services:check-summaries` validates the generated Sentry release prerequisite, Sentry Android warning, Firebase, CodePush release path, CodePush migration readiness, CodePush removal readiness, push-notification bridge, and iOS release readiness summaries as one aggregate gate before future release-service dependency or runtime changes. The Firebase summary records live npm latest metadata for `@react-native-firebase/app` and `@react-native-firebase/messaging`, the npm repository, package-current state, Android release evidence, and the explicit `not claimed` runtime-delivery status. The push-notification bridge summary records live npm latest metadata for `@react-native-community/push-notification-ios`, dependency/installed/latest alignment, static iOS bridge readiness, and the explicit `not claimed` runtime-delivery status.

`release-services:validation:handoff:dry-run` prints the full release-services validation sequence before a release-service dependency, env, or runtime change is claimed ready. The executable sequence refreshes Android release APK evidence with Sentry auto-upload disabled, refreshes Sentry/Firebase/CodePush/push/iOS summaries, and finishes with the aggregate `release-services:check-summaries` gate. Use `--skip-android-release` only when the current Android release summary is already known to match the current release inputs.

`sentry:release:validation:handoff:dry-run` prints the Sentry-specific source-map prerequisite handoff. The executable sequence validates the Sentry properties generator, optionally refreshes Android release APK evidence with Sentry auto-upload disabled, generates root/Android/iOS Sentry properties from `SENTRY_AUTH_TOKEN`, refreshes the Sentry release prerequisite summary, and finishes with the aggregate release-services summary gate. `check:sentry-release-validation-handoff-guard` keeps the command order and secret-safe rendering from drifting. Use `--skip-android-release` only when current Android release evidence is already fresh.

`codepush:update:validation:handoff:dry-run` prints the CodePush-specific update-validation handoff. The executable sequence optionally refreshes Android release APK evidence with Sentry auto-upload disabled, refreshes CodePush release-path, migration-readiness, and removal-readiness summaries, validates aggregate release-services summaries, and then refuses to claim readiness unless the release-path summary is ready for update validation, deployment-key values stayed hidden, update validation is still explicitly unclaimed, and the App Center retirement migration requirement remains visible. `check:codepush-update-validation-handoff-guard` keeps the command order and secret-safe rendering from drifting. Use `--skip-android-release` only when current Android release evidence is already fresh.
