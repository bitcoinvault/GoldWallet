...
Use `corepack yarn rn:baseline:preflight` before changing React Native package versions. It runs the lightweight Android gate plus the Metro runtime, RN upgrade path, RN target snapshot, offline target comparison guard, React 19 impact audit, QR camera migration, Sentry warning/source-map readiness, Firebase release-service, CodePush release-path, and push-notification bridge audits as a single RN-baseline readiness pass.
