...
`android:dev:check-light` runs the Android warning baseline guard, Android warning artifact guard, camera usage guard, QR scanner caller guard self-check, QR scanner caller inventory guard, Sentry usage guard, nodeify shim guard, TypeScript check, and diff whitespace check.
