...
Latest iOS static readiness evidence refresh: on 2026-06-17, `feature/bem-37-713-ios-static-readiness-refresh` re-ran static release readiness, macOS prerequisite, all-scheme handoff dry-run, validation handoff summary, and Podfile.lock refresh-plan checks for RN `0.86.0`. Static iOS files remain valid, removed-pod references remain at `0`, and runtime/archive validation remains not claimed on Windows because macOS with Xcode `16.1+`, CocoaPods, and a refreshed `ios/Podfile.lock` are still required. The current `ios/Podfile.lock` drift count is 12 active package-vs-pod entries.
