...
Latest iOS static readiness evidence refresh: on 2026-06-23, `feature/bem-37-769-ios-static-readiness-refresh` re-ran static release readiness, macOS prerequisite, Podfile.lock refresh-plan, preflight dry-run, and validation handoff summary checks for RN `0.86.0`. Static iOS files remain valid, eight guarded schemes are tracked, removed-pod references remain at `0`, Sentry bundle/source-map and dSYM phases remain present, and runtime/archive validation remains not claimed on Windows because macOS with Xcode `16.1+`, CocoaPods, and a refreshed `ios/Podfile.lock` are still required. The current `ios/Podfile.lock` drift count is 12 active package-vs-pod entries, and the combined iOS handoff summary remains `Implementation ready: no` until those macOS-only steps pass.
