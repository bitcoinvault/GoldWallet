...
`release-services:validation:handoff:dry-run` prints the full release-services validation sequence before a release-service dependency, env, or runtime change is claimed ready. The executable sequence refreshes Android release APK evidence with Sentry auto-upload disabled, runs the Android release embedded smoke, validates the release-smoke summary, refreshes Sentry warning, RN bundle compatibility, release prerequisite, Firebase, CodePush, push, iOS release/prerequisite summaries, and the combined iOS validation handoff summary, renders and validates the CodePush decision handoff, and finishes with the aggregate `release-services:check-summaries` gate. The default CodePush decision is the current post-removal posture, `remove` with `beta-has-no-ota`; pass explicit CodePush decision flags only for a future replacement branch or temporary legacy exception. Use `--skip-android-release` only when the current Android release build, manifest, and release-smoke evidence are already known to match the current release inputs.
