...
Latest iOS static readiness evidence refresh: on 2026-06-29, `feature/bem-37-816-firebase-25-1-refresh` re-ran static release readiness after the Firebase `25.1.0` package-family upgrade. Static iOS files remain valid, eight guarded schemes are tracked, removed-pod references remain at `0`, Sentry bundle/source-map and dSYM phases remain present, and runtime/archive validation remains not claimed on Windows because macOS with Xcode `16.1+`, CocoaPods, and a refreshed `ios/Podfile.lock` are still required. The current `ios/Podfile.lock` drift count is 12 active package-vs-pod entries, including `RNFBApp 12.7.5` versus `@react-native-firebase/app 25.1.0`, and the combined iOS handoff summary remains `Implementation ready: no` until those macOS-only steps pass. Earlier `feature/bem-37-769-ios-static-readiness-refresh` established the same Windows-safe iOS handoff pattern before the Firebase `25.0.1` refresh.
