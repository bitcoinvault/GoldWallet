...
`android:toolchain-target:audit` records the currently validated Android toolchain and the live latest stable AGP, current Gradle, and latest Kotlin targets. The current RN `0.86.0` baseline intentionally stays on AGP `8.13.2`, Gradle `8.13`, and Kotlin `2.1.20` because the checked AGP `9.2.1` path requires Gradle `9.4.1+`, while Gradle `9.4.1` and `9.6.0` fail compiling the included React Native Gradle plugin against newer embedded Kotlin runtime metadata.
