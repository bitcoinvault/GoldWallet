...
The 2026-06-12 generated latest snapshot confirms the tracked storage/network/config package group remains on the checked current lines used by this RN `0.86.2` baseline. `react-native-get-random-values@2.0.0` remains the current random-value provider and peers `react-native >=0.81`; the deprecated `react-native-randombytes` bridge remains removed.
...
- The secure-storage handoff validates the Keychain-primary migration-window posture, rejects restoration of the third-party package or legacy writes, and requires fallback reads/cleanup to remain active until cross-platform rollout proof exists.
- The 2026-08-11 Android upgrade-in-place evidence guards migration and post-write cleanup of legacy-only PIN, transaction-password, encrypted flag, and wallet data through the first-party bridge. Its run ID and candidate/source hashes prevent reuse after code or APK drift. Confirmed Keychain misses may migrate; Keychain read failures skip fallback, and deletion markers prevent failed legacy cleanup from restoring deleted values. iOS runtime migration and migration-release adoption remain unproven, so fallback removal is not ready.
...
- `tests/integration/Storage.test.js` locks the Keychain-first encrypted-wallet contract, including historical fallback migration, write-failure retention, cleanup, and missing reads.
