...
- `react-native-svg` is on checked `12.5.1` after `BEM-36.57`, paired with `react-native-qrcode-svg@6.1.2` to keep the QR renderer peer dependency aligned on the current RN baseline.
