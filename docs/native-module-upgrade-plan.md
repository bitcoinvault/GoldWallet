...
- `@react-native-firebase/crashlytics` -> `26.2.0`
- `@react-native-firebase/messaging` -> `26.2.0`
...
- Keep Firebase grouped by package family; after `BEM-37.965` the current family is `26.2.0`, with New Architecture enabled and Android debug build, release evidence, and embedded smoke validation required before merge. iOS remains on the explicit CocoaPods path through `$RNFirebaseDisableSPM = true` until a dedicated macOS/Xcode milestone proves the RN Firebase SPM archive flow.
...
- `@sentry/react-native` is on latest checked `8.22.0` and release tooling pins and deduplicates `@sentry/cli@3.6.2`; the prerequisite audit verifies the Android resolver reaches that root package. The versioned SDK patch remains required because upstream `8.22.0` still contains two source-map helper exit typos. A credentialed Android dev upload was validated on the previous SDK baseline, but upload ingest and symbolication remain unclaimed for `8.22.0` until its Gradle upload and event-symbolication canaries are rerun. Future Sentry work should also complete macOS/Xcode/CocoaPods iOS dSYM validation after `ios/Podfile.lock` drift is refreshed. Use `corepack yarn sentry:release:validation:preflight` for repeatable release readiness checks.
...
- Latest checked on 2026-08-10: `react-native-camera-kit@18.0.1`, `react-native-permissions@5.6.1`, `react-native-vision-camera@5.2.2`, `react-native-qrcode-svg@6.3.21`, `react-native-svg@15.15.5`, and `qrcode@1.5.4`.
