...
- `@sentry/react-native` is on latest checked `8.13.0` and release tooling now pins `@sentry/cli@3.5.0`; future Sentry work should focus on release source-map/dSYM validation with `SENTRY_AUTH_TOKEN` and generated `sentry.properties`, not another blind package bump.
