...
- `@sentry/react-native` is on latest checked `8.14.0` and release tooling now pins direct `@sentry/cli@3.5.1`; future Sentry work should focus on release source-map/dSYM validation with `SENTRY_AUTH_TOKEN` and generated `sentry.properties`, not another blind package bump.
