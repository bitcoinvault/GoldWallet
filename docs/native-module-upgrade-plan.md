...
- `@sentry/react-native` is on latest checked `8.14.0` and release tooling now pins direct `@sentry/cli@3.5.1`; future Sentry work should focus on release source-map/dSYM validation with `SENTRY_AUTH_TOKEN`, generated `sentry.properties`, and macOS/Xcode/CocoaPods iOS validation after `ios/Podfile.lock` drift is refreshed, not another blind package bump. Use `corepack yarn sentry:release:validation:handoff --preflight-only --skip-android-release` for non-secret Sentry release readiness checks when release evidence is already fresh but credentials are unavailable.
