...
- Add offline transaction-construction coverage for the native Bech32 HD wallet before deeper wallet crypto dependency migrations.
- Lock signed regular-send behavior from a deterministic BIP84 UTXO fixture.
- Lock signed send-max behavior through the `coinselect/split` path using deterministic BIP84 UTXO fixtures.
- Extend the wallet crypto runtime audit so future branches keep this transaction coverage present.

Findings:

- `HDSegwitBech32Wallet.createTransaction` signs deterministic BTCV Bech32 UTXOs without Electrum when the UTXO address is generated by the wallet.
- The regular-send fixture preserves target and change output addresses; the send-max fixture preserves a single recipient output equal to selected input value minus fee.
...
- Probe `react@19.2.6` and `react-test-renderer@19.2.6`, the latest stable npm releases checked for this branch.
- Reject the patch-level React update on the current React Native `0.85.3` baseline because Android runtime smoke shows a redbox: bundled `react-native-renderer` stays on `19.2.3` and requires the exact same React version.
- Keep the committed React baseline at `19.2.3` until the next React Native baseline moves the renderer with it.
- Ignore the Android CMake `.cxx` output generated by the RN `0.85.3` Android debug build.
- Refresh the RN upgrade-path preflight guard after the iOS release-readiness guard was added to `rn:baseline:preflight`.

Why:

- React is part of the RN runtime surface, so patch updates must be validated against the bundled `react-native-renderer`, not just npm peer ranges.
- This branch records the failed latest-first probe and prevents the incompatible React patch from being committed as a "successful" modernization step.

Validation:

- `npm view react version dist-tags peerDependencies dependencies engines --json`
- `npm view react-test-renderer version peerDependencies dependencies engines --json`
- `corepack yarn add react@19.2.6 react-test-renderer@19.2.6`
- `corepack yarn remove false`
- `corepack yarn postinstall`
- `JAVA_HOME=D:\tmp\jdks\temurin17\jdk-17.0.19+10 corepack yarn android:dev:assemble`
- `JAVA_HOME=D:\tmp\jdks\temurin17\jdk-17.0.19+10 corepack yarn android:dev:smoke:embedded` failed with incompatible React/runtime renderer versions, so the React package change was reverted.
- `corepack yarn add react@19.2.3 react-test-renderer@19.2.3`
- `corepack yarn postinstall`
- `corepack yarn react:package-coupling:audit`
- `corepack yarn check:react-package-coupling-guard`
...
- Re-check and update the browserified `events` polyfill from `1.1.1` to latest stable `3.3.0`.
- Keep app source, test source, native project files, Metro config, and rn-nodeify shim code unchanged.
- Treat this as a runtime/bundler polyfill dependency update because `events` is installed by the rn-nodeify polyfill step and is required by the patched `stream-browserify` path.

Findings:

- `npm view events version dist-tags engines dependencies peerDependencies --json` reports `latest` as `3.3.0`.
- The app's own event registry remains the local `events.js`; the npm `events` package is used as the Node EventEmitter polyfill surface.
- `events@3.3.0` has no runtime dependencies and remains compatible with the existing rn-nodeify shim setup.
...
- Re-check and update the browserified `util` polyfill from `0.12.1` to latest stable `0.12.5`.
- Keep app source, test source, native project files, Metro config, and rn-nodeify shim code unchanged.
- Treat this as a runtime/bundler polyfill dependency update because `util` is installed by the rn-nodeify polyfill step and is also pulled transitively by the modernized `assert` package.

Findings:

- `npm view util version dist-tags engines dependencies peerDependencies --json` reports `latest` as `0.12.5`.
- The lockfile now resolves the direct `util` dependency and the `assert@2.1.0` transitive `util` dependency to the same `0.12.5` package.
- `rn-nodeify` reapplied the existing polyfill normalization and `corepack yarn check:rn-nodeify-shims` remained stable.
...
- Update `react-native-webview` from `11.26.1` to latest `13.16.1`.
- Keep the existing Terms and Conditions WebView call sites unchanged.
- Refresh the native-module inventory, native-module upgrade plan, and storage/network audit references.

Findings:

- `npm view react-native-webview version dist-tags engines peerDependencies --json` reports `latest` as `13.16.1`.
- `react-native-webview@13.16.1` keeps broad `react` and `react-native` peer compatibility.
- Android `assembleDevDebug` confirms the updated native module compiles on the RN `0.81.6` baseline.
...
- `package.json` now has `react-native-gesture-handler@2.29.1`, `react-native-screens@4.24.0`, `react-native-safe-area-context@5.8.0`, `react-native-svg@15.15.5`, `react-native-share@12.3.1`, `react-native-vector-icons@10.3.0`, `react-native-webview@13.16.1`, and `react-native-fast-image@8.6.3`.
...
- Upgrade `@react-native-community/slider` from `2.0.0-rc.1` to `5.2.0`.
- Remove the package's two Android `jcenter()` findings from the warning baseline because `5.2.0` uses `google()` and `mavenCentral()`.
- Add Slider to the native module inventory guard and native module upgrade plan.
- Keep app source unchanged; current app code has no Slider imports.

Why:

- The old Slider package was a direct native dependency and contributed two Android warning sources despite not being imported by app code.
- Updating the package keeps the module available for any native/project references while removing both deprecated repository declarations.

Validation:

- `JAVA_HOME=D:\tmp\jdks\temurin17\jdk-17.0.19+10 corepack yarn android:dev:audit-warnings` passed and reported `Targeted Android Gradle warnings: 6` with `Unexpected targeted Android Gradle warnings: 0`.
- `corepack yarn android:dev:check-warning-guard` passed.
- `corepack yarn android:dev:check-artifact-guard` passed.
- `corepack yarn android:dev:check-warning-audit-summary` passed.
...
### BEM-37.107 - React Native Localize jcenter cleanup

- Branch: `feature/bem-37-localize-jcenter-cleanup`
- Parent branch: `upgrade/wallet-modernization`

Scope:

- Upgrade `react-native-localize` from `1.4.3` to `3.7.0`.
- Remove the package from the Android warning baseline because `3.7.0` uses `google()` and `mavenCentral()` instead of `jcenter()`.
- Refresh the native module inventory, storage/network compatibility audit, warning baseline guard, and warning-summary guard fixtures for the new eight-warning RN `0.76` baseline.
- Keep the app language-selection code and existing `RNLocalize.getLocales()` call unchanged.

Why:

- `react-native-localize@1.4.3` was one of the RN `0.76` Android warning sources because its Android Gradle file still declared `jcenter()`.
- The app uses this package through a narrow localization startup surface, so this is a focused dependency cleanup with emulator validation instead of a broad i18n rewrite.

Validation:

- `JAVA_HOME=D:\tmp\jdks\temurin17\jdk-17.0.19+10 corepack yarn android:dev:audit-warnings` passed and reported `Targeted Android Gradle warnings: 8` with `Unexpected targeted Android Gradle warnings: 0`.
- `corepack yarn android:dev:check-warning-guard` passed.
- `corepack yarn android:dev:check-artifact-guard` passed.
- `corepack yarn android:dev:check-warning-audit-summary` passed.
- `corepack yarn check:native-module-inventory` passed.
...
- Extend the Android smoke helper with optional empty-state tab navigation validation.
- Enable the tab navigation check for both debug and signed release embedded smoke.
- Validate bottom navigation from the empty Wallets dashboard through Authenticator, Address book, Settings, and back to Wallets before the final screenshot.
- Update smoke summary validation and workflow docs so tab-navigation proof is recorded and guarded.

Findings:

- The previous smoke proved startup, onboarding, empty Wallets dashboard, and Create/Import CTA navigation, but did not prove that the other bottom-tab screens still mounted cleanly after the React Native modernization work.
- The new flow checks stable resource IDs on each empty-state tab: `no-authenticators-icon`, `contacts-searchbar`, `no-contacts-icon`, `goldwallet-logo`, key Settings rows, and all bottom navigation tab IDs.
- Debug embedded smoke passed on `emulator-5554`, completed onboarding, validated CTA navigation, validated tab navigation, returned to Wallets, and recorded `Validated empty-tab navigation: yes`.
- Signed dev release embedded smoke also passed on `emulator-5554` with the same tab navigation proof and final Wallets dashboard screenshot.
- No UI runtime code, native code, dependency versions, package scripts, or Metro behavior changed in this branch; the app-facing proof is the broader smoke runtime check.
...
- Add an isolated `bl@latest` compatibility probe to the BL readiness audit and generated summary guard.
- Upgrade `@types/react` from `19.2.16` to `19.2.17` and align the React type impact, package coupling, foundation, and strategy guard baselines.
- Refresh the direct-outdated and foundation docs so blocked latest dependencies remain backed by concrete compatibility evidence.

Findings:

- The first online direct-outdated snapshot reported new review-required drift for `@types/react@19.2.17`; the branch applies that patch update and returns `Review-required entries` to `0`.
- The isolated probe confirms `bl@7.0.3` works through ESM import, but bare CommonJS `require("bl")` and `bl/package.json` subpath access both fail with `ERR_PACKAGE_PATH_NOT_EXPORTED`.
- Because legacy transitive code still needs CommonJS/package metadata access, `bl` remains pinned through the `6.1.6` resolution instead of moving to latest.
- The full online preflight passes after refreshing Android release evidence for the package and lockfile change.
- Android debug smoke and signed dev release smoke both pass on `emulator-5554`.
- The first signed release smoke attempt hit a transient `uiautomator dump` exit `137`; the app stayed alive, a manual UI dump succeeded immediately afterwards, and the rerun completed without fatal Android or React Native runtime errors.
...
- `PATH=D:\tmp\node\node-v24.16.0-win-x64;%PATH% corepack yarn check:react19-impact-guard`
- `PATH=D:\tmp\node\node-v24.16.0-win-x64;%PATH% corepack yarn react19:impact:audit`
...
- `PATH=D:\tmp\node\node-v24.16.0-win-x64;%PATH% JAVA_HOME=D:\tmp\jdks\temurin17\jdk-17.0.19+10 corepack yarn android:dev:release:verify-local`
- `PATH=D:\tmp\node\node-v24.16.0-win-x64;%PATH% JAVA_HOME=D:\tmp\jdks\temurin17\jdk-17.0.19+10 corepack yarn android:dev:release:check-summary`
- `PATH=D:\tmp\node\node-v24.16.0-win-x64;%PATH% JAVA_HOME=D:\tmp\jdks\temurin17\jdk-17.0.19+10 corepack yarn android:dev:release:check-apk-manifest`
- `PATH=D:\tmp\node\node-v24.16.0-win-x64;%PATH% JAVA_HOME=D:\tmp\jdks\temurin17\jdk-17.0.19+10 corepack yarn rn:baseline:preflight:online`
- `PATH=D:\tmp\node\node-v24.16.0-win-x64;%PATH% corepack yarn lint:baseline:audit` passed with the existing ESLint baseline
- `PATH=D:\tmp\node\node-v24.16.0-win-x64;%PATH% corepack yarn test:unit --runInBand`
- `PATH=D:\tmp\node\node-v24.16.0-win-x64;%PATH% corepack yarn test:storage-network:focused`
- `PATH=D:\tmp\node\node-v24.16.0-win-x64;%PATH% JAVA_HOME=D:\tmp\jdks\temurin17\jdk-17.0.19+10 corepack yarn android:dev:assemble`
- `PATH=D:\tmp\node\node-v24.16.0-win-x64;%PATH% JAVA_HOME=D:\tmp\jdks\temurin17\jdk-17.0.19+10 corepack yarn android:dev:smoke:embedded`
- `PATH=D:\tmp\node\node-v24.16.0-win-x64;%PATH% JAVA_HOME=D:\tmp\jdks\temurin17\jdk-17.0.19+10 corepack yarn android:dev:release:smoke:embedded` passed on rerun after a transient `uiautomator dump` failure
- `PATH=D:\tmp\node\node-v24.16.0-win-x64;%PATH% JAVA_HOME=D:\tmp\jdks\temurin17\jdk-17.0.19+10 corepack yarn android:dev:release:check-smoke-summary`
- `PATH=D:\tmp\node\node-v24.16.0-win-x64;%PATH% corepack yarn check:rn-nodeify-shims`
- `PATH=D:\tmp\node\node-v24.16.0-win-x64;%PATH% corepack yarn typescript:check`
- `PATH=D:\tmp\node\node-v24.16.0-win-x64;%PATH% corepack yarn check:modernization-log-ids`
- `git diff --check`
