...
- The app uses `@sentry/react-native@8.22.0` with direct release tooling on `@sentry/cli@3.6.2`.
...
- `corepack yarn sentry:rn-bundle-task-compat:audit` records the static compatibility status between Sentry `8.22.0` bundle-task extraction and the RN `0.87.0` `BundleHermesCTask` property model.
- `corepack yarn sentry:rn-bundle-task-compat:check-summary` validates the generated local compatibility summary.
- The active RN `0.87.0` Android warning audit no longer reports Sentry `execResult`; Sentry still needs release/source-map validation with local credentials before claiming the SDK/tooling change complete for release artifacts.
- The latest npm releases checked for the Sentry release path on 2026-08-11 are `@sentry/react-native@8.22.0`, direct `@sentry/cli@3.6.2`, and Sentry-scoped `undici@8.10.0`. The SDK is current on the RN `0.87.0` baseline, the release CLI is pinned explicitly as dev tooling, and the transport requires Node `>=22.19.0`, satisfied by the repo Node `24.16.0` baseline.
- `corepack yarn check:sentry-cli-transport` resolves `undici` from the Sentry CLI and jsdom owner contexts, verifies the CLI's declared `undici@^6.22.0` range and CommonJS `fetch`/`ProxyAgent`/`request`, exercises local HTTP fetch plus the response-stream conversion used by the fallback installer, and checks the active packaged CLI binary/version without contacting Sentry or uploading artifacts. A real fallback CDN download and credentialed upload remain separate gates.
...
- `@sentry/react-native@8.22.0` declares `react-native >=0.65.0`, but release artifact behavior still has to be proven instead of disabling source-map upload. The published package still needs the repo-owned `patch-package` correction for two `process.exist(1)` typos in `scripts/has-sourcemap-debugid.js`.
...
- Android release Gradle output on Sentry `8.22.0` is checked for `Could not extract bundle task arguments` after the repo-owned RN `0.87.0` bundle task args shim; final source-map upload validation still requires a credentialed upload run.
- Static compatibility evidence shows Sentry expects `jsIntermediateSourceMapsDir` as a `Directory`, while RN `0.87.0` exposes it on `BundleHermesCTask` as a `RegularFileProperty`; Sentry's fallback also expects an `args` property that the RN task does not expose. Do not patch `node_modules` or add unsupported dynamic task properties in `android/app/build.gradle`.
...
- iOS static readiness remains valid, but iOS macOS archive validation is not ready on this Windows host; `ios/Podfile.lock` has 13 active drift issues, including `react-native-safe-area-context 3.3.2` versus package `5.8.1` and `RNScreens 3.6.0` versus package `4.27.0`.
...
- Keep the newest compatible `@sentry/react-native` line for the current React Native `0.87.0` baseline.
