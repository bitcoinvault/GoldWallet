...
- iOS static readiness remains valid, but iOS macOS archive validation is not ready on this Windows host; `ios/Podfile.lock` has 13 active drift issues, including `react-native-safe-area-context 3.3.2` versus package `5.9.0` and `RNScreens 3.6.0` versus package `4.27.0`.
