...
- The prerequisite audit records per-file readiness for the three required Sentry properties files, validates that `create-sentry-properties.sh` writes all three paths, and confirms the expected non-secret static defaults without printing token values.
