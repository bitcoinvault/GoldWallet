...
- The latest legacy `react-native-camera` package checked on 2026-06-12 is still `4.2.1`, and it still does not solve the dependency-owned Android `jcenter()` warning cleanly.
...
- Current latest package checked on 2026-06-12 is `react-native-vision-camera@5.0.11`.
...
- Current latest package checked on 2026-06-12 is `react-native-camera-kit@18.0.0`.
- The camera candidate audit verifies this against live npm metadata before future scanner work.
- Node requirement `>=18` is compatible with the current Node 24 modernization baseline.
- Live peer ranges checked on 2026-06-12 remain `react@*` and `react-native@*`, so the installed CameraKit scanner remains the highest current compatible target without adding a new native peer stack.
- Current proof role: selected implementation for the QR scanner migration.

### QR Rendering Pair

- Current latest package checked on 2026-06-12 is `react-native-qrcode-svg@6.3.21`.
- Current latest native renderer package checked on 2026-06-12 is `react-native-svg@15.15.5`.
- Current latest encoder package checked on 2026-06-12 is `qrcode@1.5.4`.
- Live `react-native-qrcode-svg@6.3.21` peer ranges checked on 2026-06-12 remain `react@*`, `react-native@>=0.63.4`, and `react-native-svg@>=14.0.0`; the direct dependency set remains `prop-types@^15.8.0`, `qrcode@^1.5.4`, and `text-encoding@^0.7.0`.
