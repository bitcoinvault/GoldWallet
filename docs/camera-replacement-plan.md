...
- The latest legacy `react-native-camera` package checked on 2026-07-10 is still `4.2.1`, and it still does not solve the dependency-owned Android `jcenter()` warning cleanly.
...
- `corepack yarn camera:qr-validation:handoff --include-android-release-smoke` extends the Camera/QR handoff with local devRelease build evidence, release APK smoke, release create-wallet smoke, and both release smoke summary checks for scanner-affecting release branches.
...
- The 2026-07-10 Camera/QR readiness refresh confirmed `react-native-camera-kit@18.0.0`, `react-native-qrcode-svg@6.3.21`, `react-native-svg@15.15.5`, and `qrcode@1.5.4` are still current npm latest packages, focused scanner/render unit tests pass, and VisionCamera latest moved to `react-native-vision-camera@5.1.0` while still requiring the Nitro native peer stack. No runtime/native/package change was made in that refresh, so no new emulator smoke was required.
- The 2026-07-11 Camera/QR controlled release blocker refresh confirmed the same live latest package targets and added release-proof classification to `camera:qr-validation:summary`. When Android release smoke/create-wallet proof is blocked by the classified dev/testnet Electrum certificate expiry, the summary now reports `Camera/QR release runtime proof state: blocked-by-electrum-certificate-expired` instead of treating the release proof as ready. Full Android dev/release QR scanner proof still requires rerunning the smoke flows after the certificate is renewed.
...
- Current latest package checked on 2026-07-10 is `react-native-vision-camera@5.1.0`.
...
- Current latest package checked on 2026-07-10 is `react-native-camera-kit@18.0.0`.
- The camera candidate audit verifies this against live npm metadata before future scanner work.
- Node requirement `>=18` is compatible with the current Node 24 modernization baseline.
- Live peer ranges checked on 2026-07-10 remain `react@*` and `react-native@*`, so the installed CameraKit scanner remains the highest current compatible target without adding a new native peer stack.
- Current proof role: selected implementation for the QR scanner migration.

### QR Rendering Pair

- Current latest package checked on 2026-07-10 is `react-native-qrcode-svg@6.3.21`.
- Current latest native renderer package checked on 2026-07-10 is `react-native-svg@15.15.5`.
- Current latest encoder package checked on 2026-07-10 is `qrcode@1.5.4`.
- Live `react-native-qrcode-svg@6.3.21` peer ranges checked on 2026-07-10 remain `react@*`, `react-native@>=0.63.4`, and `react-native-svg@>=14.0.0`; the direct dependency set remains `prop-types@^15.8.0`, `qrcode@^1.5.4`, and `text-encoding@^0.7.0`.
...
- `corepack yarn camera:qr-validation:handoff --include-android-release-smoke` for scanner-affecting release branches or when current release APK evidence must prove the QR scanner screen.
