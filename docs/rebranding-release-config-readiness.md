...
Checked on 2026-06-24 from the current `upgrade/wallet-modernization` baseline. This document does not change runtime behavior. It records the app identity, release-service, store metadata, and explorer surfaces that must move together when the GoldWallet rebrand starts.
...
- Android runtime smoke for explorer/Electrum behavior is currently blocked by an expired dev testnet Electrum TLS certificate; do not claim runtime explorer validation until the endpoint is renewed or changed and smoke passes again.
