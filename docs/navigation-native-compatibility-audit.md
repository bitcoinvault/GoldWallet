...
This audit supports staged navigation and layout-native dependency work on the RN `0.87.0` baseline.
...
- `react-native-gesture-handler@3.1.0` is the latest checked stable line validated with the RN `0.87.0` New Architecture baseline. The earlier RN `0.76.9` Kotlin/codegen blocker no longer reproduces after the RN foundation.
- `react-native-screens@4.27.0` is the latest checked stable screens baseline. Its npm peer metadata accepts the current React and React Native runtime, and the package is validated against the RN `0.87.0` checkpoint.
- `react-native-safe-area-context@5.9.0` is the latest stable safe-area baseline. Upstream adds AGP `9.2.1` compatibility and web resize/nested-provider fixes while retaining unrestricted React and React Native peer ranges.
...
- Navigation 7 plus masked-view removal is validated on Android through the RN `0.87.0` proof branch and emulator smoke.
...
- The previous `react-native-screens@4.25.x` blocker is cleared by the RN `0.87.0` checkpoint; future navigation-native dependency jumps should stay tied to Android navigation smoke validation.
- Safe Area Context `5.9.0` adopts AGP `9.2.1`, matching this repo's Android plugin baseline, and separates generated Java from Fabric/Paper Kotlin source sets. Android build plus stack/tab and `ScreenTemplate` layout smoke remain mandatory because this is a native package update.
...
- Keep `react-native-gesture-handler@3.1.0`, `react-native-screens@4.27.0`, `react-native-safe-area-context@5.9.0`, and `react-native-bootsplash@7.3.2` fixed until the next navigation-native validation branch.
