...
- iOS `Podfile.lock` is stale on Windows and still references removed/older native pods, including `RNCMaskedView` from the removed `@react-native-community/masked-view` package; refresh it on macOS before claiming iOS validation.
