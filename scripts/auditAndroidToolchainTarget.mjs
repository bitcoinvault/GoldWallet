...
const minimumGradleByAgpMinor = {
  '9.2': '9.4.1',
  '9.3': '9.5.0',
};
const getMinimumGradleForAgp = agp => minimumGradleByAgpMinor[agp.split('.').slice(0, 2).join('.')] || '';
...
const getProbeEvidence = ({ log, directAgp9Probe, currentAgp, currentGradle, currentKotlin, rnGradlePlugin }) => {
  const evidenceEntry = getLogEntry(log, 'BEM-37.976');
  const requiredSnippets = [
    ...
    `Kotlin metadata \`${directAgp9Probe.kotlinRuntimeMetadata}\``,
    `up to \`${directAgp9Probe.rnKotlinMetadataCeiling}\``,
  ...
  return {
    status: evidenceEntry && missingSnippets.length === 0 ? 'recorded' : 'missing-or-stale',
    requiredSnippets,
    missingSnippets,
  };
};
...
export const collectAndroidToolchainTargetAudit = async () => {
  ...
  const currentAgpMinimumGradle = getMinimumGradleForAgp(currentAgp);
  const latestAgpMinimumGradle = getMinimumGradleForAgp(latestStableAgp);
  ...
  const directAgp9Probe = {
    ...
    kotlinRuntimeMetadata: '2.4.0',
    rnKotlinMetadataCeiling: '2.3.0',
    evidence: 'docs/wallet-modernization-log.md BEM-37.976',
  };
  ...
  return {
    ...
    currentGradle,
    latestGradle,
    currentAgpMinimumGradle,
    latestAgpMinimumGradle,
    currentKotlin,
    latestKotlin,
    ...
    blocked: true,
    blockers: [
      `AGP ${latestStableAgp} requires Gradle ${latestAgpMinimumGradle} or newer.`,
      `The Gradle ${latestAgpMinimumGradle}+ path is blocked: direct AGP ${directAgp9Probe.agp} / Gradle ${directAgp9Probe.gradle} / Kotlin ${directAgp9Probe.kotlin} probe failed in ${directAgp9Probe.task} while compiling ${directAgp9Probe.source}; Gradle loaded Kotlin runtime metadata ${directAgp9Probe.kotlinRuntimeMetadata}, but the React Native Gradle plugin ${rnGradlePlugin} compiler path can read up to metadata ${directAgp9Probe.rnKotlinMetadataCeiling}.`,
      `The validated Android baseline remains AGP ${getClasspathVersion(androidBuildGradle, 'com.android.tools.build:gradle')}, Gradle ${getGradleWrapperVersion(
         gradleWrapper,
      )}, Kotlin ${getQuotedGradleValue(androidBuildGradle, 'kotlinVersion')}, compile SDK 37, target SDK 36, and JDK 17 until a newer React Native Gradle plugin baseline clears the blocker.`,
    ],
  };
};
...
export const formatAndroidToolchainTargetSummary = (audit, generatedAt = new Date().toISOString()) =>
  [
    ...
    `Current AGP minimum Gradle wrapper: ${audit.currentAgpMinimumGradle}`,
    `Latest AGP minimum Gradle wrapper: ${audit.latestAgpMinimumGradle}`,
    ...
    `Required action: keep the validated AGP ${audit.currentAgp} / Gradle ${audit.currentGradle} Android baseline until a React Native Gradle plugin baseline can compile against Gradle ${audit.latestGradle} Kotlin metadata, then rerun Android assemble, release validation, and emulator smoke.`,
    '',
  ].join('\n');
