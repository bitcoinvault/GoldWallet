...
if (devDependencies['@typescript-eslint/eslint-plugin'] !== '8.61.0') {
  errors.push(
    `@typescript-eslint/eslint-plugin must stay on 8.61.0 for this compatibility guard. Found ${
      devDependencies['@typescript-eslint/eslint-plugin'] || '<missing>'
    }`,
  );
}

if (devDependencies['@typescript-eslint/parser'] !== '8.61.0') {
  errors.push(
    `@typescript-eslint/parser must stay on 8.61.0 for this compatibility guard. Found ${devDependencies['@typescript-eslint/parser'] || '<missing>'}`,
  );
}
