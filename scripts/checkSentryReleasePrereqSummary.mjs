...
if (!existsSync(summaryPath)) {
  console.error(`Missing Sentry release prerequisite summary artifact: ${summaryPath}`);
  process.exit(1);
}
...
if (errors.length > 0) {
  console.error('Sentry release prerequisite summary artifact is invalid:');
  errors.forEach(error => console.error(`- ${error}`));
  process.exit(1);
}

console.log('Sentry release prerequisite summary artifact is valid.');
