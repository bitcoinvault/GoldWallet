...
export const expectedReactNativeBaselinePreflight =
  'yarn android:dev:check-light && yarn metro:dev-runtime:audit && yarn check:node-runtime-transition-guard && yarn node:runtime-transition:audit && yarn upgrade:strategy:audit && yarn rn:upgrade-path:audit && yarn rn:076-foundation:audit && yarn check:rn-target-snapshot-guard && yarn check:rn-target-snapshot-current-guard && yarn rn:target-snapshot:audit && yarn check:react19-impact-guard && yarn react19:impact:audit && yarn check:react-package-coupling-guard && yarn react:package-coupling:audit && yarn check:test-type-coupling-guard && yarn test:type-coupling:audit && yarn camera:candidate:audit && yarn camera:candidate:check-summary && yarn camera:qr-migration:audit && yarn camera:qr-migration:check-summary && yarn masked-view:migration:audit && yarn masked-view:migration:check-summary && yarn secure-storage:migration:audit && yarn secure-storage:migration:check-summary && yarn sentry:android-warning:audit && yarn sentry:android-warning:check-summary && yarn check:android-warning-source-summaries-guard && yarn android:dev:check-warning-source-summaries && yarn check:android-warning-audit-summary-guard && yarn android:dev:check-warning-audit-summary && yarn check:android-smoke-summary-guard && yarn android:dev:check-smoke-summary && yarn sentry:release:prereq-audit && yarn sentry:release:prereq-check-summary && yarn firebase:release-services:audit && yarn firebase:release-services:check-summary && yarn codepush:release:path-audit && yarn codepush:release:path-check-summary && yarn push-notification:bridge-audit && yarn push-notification:bridge-check-summary && yarn ios:release:readiness:audit && yarn ios:release:readiness:check-summary && yarn check:release-services-summary-guard && yarn release-services:check-summaries';
