...
export const getBabel8MigrationProbeSummaryErrors = summary => {
  ...
  if (reactNativeVersion !== '0.87.0') {
    errors.push(`React Native version must stay 0.87.0 for this blocker evidence. Received: ${reactNativeVersion || 'missing'}`);
  }

  if (rnBabelPresetVersion !== '0.87.0') {
    errors.push(
      `React Native Babel preset version must stay 0.87.0 for this blocker evidence. Received: ${rnBabelPresetVersion || 'missing'}`,
    );
  }
  ...
  if (!requiredAction.includes('keep Babel 8 blocked on RN 0.87.0')) {
    errors.push('Required action must keep Babel 8 blocked on RN 0.87.0');
  }
