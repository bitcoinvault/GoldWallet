...
This runs the lightweight Android warning, validation artifact, camera usage, QR scanner caller, QR render usage, Sentry usage, CodePush usage, Firebase usage, iOS push notification bridge, release-service env keys, Android env mapping, iOS scheme config mapping, storage/network usage, native module inventory and upgrade-plan, node polyfill shim, TypeScript, and diff whitespace guards used before the offline suites in `prepush`.
