...
* For Android: 
    * Java SE Development Kit (JDK) 17. The React Native 0.87 / AGP 9.2 baseline and Android build guard require JDK 17 exactly.
    * Android SDK
...
Android release metadata has one tracked source of truth: `android/release-version.properties`. The public Google Play baseline is recorded separately in `android/play-release-baseline.json`; the current public listing reports `6.5.2`, while this branch carries provisional Android candidate `6.5.3 (15)`. It is not yet a valid store update candidate because the private latest Play Console `versionCode` has not been supplied, so monotonicity of code `15` is unproven.
